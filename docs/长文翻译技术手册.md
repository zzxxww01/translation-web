...
**术语候选提取：** 规则保存后，`_extract_term_candidates_from_rules()` 用正则从规则文本中提取 `"X" 翻译为 "Y"` 形式的术语候选，保存到 `data/memory/{project_id}_term_candidates.json`。这些候选在术语审阅时（2.6）被合并到 prescan 结果中。

## 3.4 规则注入翻译 prompt

`memory_service.get_rules_for_prompt(project_id)` → 读取规则列表 → 截断（上限 20 条 / 600 字符，常量 `MAX_LEARNED_RULES_IN_PROMPT` 和 `MAX_LEARNED_RULES_CHARS`） → 通过 `prompt_builder._build_rules_section()` 拼入 `## 已学习的翻译规则` 区块（见 6.3.3）。
...
- **生成内容：** 最多 20 条 / 600 字符（`MAX_LEARNED_RULES_IN_PROMPT` / `MAX_LEARNED_RULES_CHARS`），自然语言规则原样输出到 `## 已学习的翻译规则`
...
确认流的重新翻译面板（`RetranslatePanelEnhanced`）预设了三个快捷指令，以固定 instruction 文本调用重译：

| 快捷指令 | 发送给模型的 `{instruction}` |
|---|---|
| **可读性** | 请从句法结构层面优化译文：拆长句（控制在15-30字）、去被动语态改主动、删多余连接词（"此外""然而"）、调整为中文语序、直接用动词不要"对……进行……"、连续三个"的"必须拆句。 |
| **更地道** | 请从表达层面优化译文：用具体事实代替空泛描述、用简单结构代替复杂绕行、不要公式化段落和同义词循环、把"基于""鉴于""旨在"换成"根据""考虑到""为了"等自然表达。 |
| **更专业** | 请优化译文的专业表达，体现semiAnalysis分析师水准：保留原文观点和判断力度不要弱化、产品/技术代号保留英文（CoWoS、HBM、EUV）行业术语用中文（晶圆代工、良率）金融术语用中文（营收、毛利率）、删除口水话和宣传腔、数据密集段落拆分重组。 |

用户也可输入自定义指令。快捷指令定义在 [RetranslatePanelEnhanced.tsx](../web/frontend/src/features/confirmation/components/panels/RetranslatePanelEnhanced.tsx)。

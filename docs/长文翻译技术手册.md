...
| #4 章节标题翻译 | preview | 0.3 | 同 #3，对应方法 `translate_all_section_titles()`（批量）/ `translate_section_title()`（单条 fallback） |
...
翻译策略（`TranslationStrategy`）：`preserve`（保留英文）/ `first_annotate`（首次加注）/ `translate`（直接翻译）
...
**FIRST_ANNOTATE 动态降级：** 通过 `term_usage` 参数（由 `TermUsageTracker` 提供），`first_annotate` 策略的术语在首次标注后自动改为纯翻译形式，避免重复标注。

## 2.3 术语预扫描（Prescan）

**Prompt：** `section_prescan.v2.txt`（#5），Flash 模型
...
**短章节跳过：** section 原文总字符数 < `PRESCAN_MIN_CHARS`（默认 300，可通过 `.env` 配置）时跳过 prescan，避免对极短章节浪费 Flash 调用。
...
**FIRST_ANNOTATE 动态降级：** `_build_glossary_section()` 接受 `term_usage` 参数（由 `TermUsageTracker.used_translations` 提供）。对于 `first_annotate` 策略的术语，如果已在之前的段落中使用过，则不再输出首次标注要求，改为 `原文：TSMC；标准写法：台积电；要求：本段直接写“台积电”`。这通过 `build_prompt()` 和 `build_retranslation_prompt()` 的 `term_usage` 参数透传。
...
- 图注翻译：`ImageProcessingService._translate_caption_if_needed()`

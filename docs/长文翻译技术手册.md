...
- `GlossaryManager` 在 API 层通过 FastAPI 依赖注入使用单例（`GlossaryManagerDep`），不再在模块级创建独立实例。
- 每次调用 `save_project()` 或 `save_global()` 时，自动递增 `glossary.version`，用于下游缓存失效判断。
...
链路 B/C 中，`LayeredContextManager._get_relevant_terms()` 增加了段落级过滤：当传入 `paragraph_text` 时，只返回在当前段落原文中出现的术语（case-insensitive 匹配）。同时合并 `terminology_version` 中 prescan 新发现的术语，并查询 `TermUsageTracker` 获取已确认的偏好翻译覆盖 `EnhancedTerm.translation`。

#### TermUsageTracker 大小写统一

`TermUsageTracker`（`core/models/analysis.py`）的 `record_usage()` 和 `get_preferred_translation()` 统一使用 `.lower()` 作为 key，避免同一术语因大小写不同被重复记录。
...
**Prescan 长内容自动分段：** 当 section 内容超过 7000 字符时，`base.py` 的 `prescan_section()` 自动按段落边界分割为多个 chunk（每个 ≤ 6000 字符），分别调用 Flash，最后合并去重 `new_terms` 和 `term_usages`。

**Prescan 结果回流：** `context_manager.add_terms_from_prescan()` 在无冲突正常添加术语时，同步回流到 `article_analysis.terminology`，使后续 section 的 prompt 可以看到前面 section prescan 发现的新术语。
...
**FIRST_ANNOTATE 自动推断：** `_infer_strategy()` 增加规则：当术语含英文字母且有不同的中文翻译时，自动推断为 `first_annotate` 策略。

**规则→术语候选合并：** `prepare_review()` 调用 `_merge_rule_candidates()`，从 `data/memory/{project_id}_term_candidates.json` 读取由学习链路提取的术语候选，合并到 prescan 结果中参与审阅。

## 2.7 术语冲突检测

**文件：** `api/routers/project_glossary.py`

新增 `POST /projects/{project_id}/glossary/check-conflict` 端点，检查术语是否与项目级或全局术语表存在同名冲突。前端在术语管理页保存新术语前调用此端点，有冲突时弹出确认对话框。

全局术语 `POST /glossary` 端点从硬拒绝改为非阻塞覆盖，响应中返回 `replaced` 字段标识是否覆盖了已有术语。

## 2.8 四步法实时术语冲突

**相关文件：**
- `services/batch_translation_service.py`
- `api/routers/translate_projects.py`
- `web/frontend/src/features/document/services/fullTranslationService.ts`
- `web/frontend/src/features/document/index.tsx`

链路 B 的 section prescan 发现术语冲突时，不再只把冲突记进 `progress.errors`。
现在会直接走一轮实时 SSE 对话：

1. 后端发送 `term_conflict` 事件。
2. 前端暂停当前翻译会话并弹出冲突对话框。
3. 对话框同时展示：
   - 已有译法 / 新译法
   - 词义说明（`existing_note` / `new_note`）
   - 上下文（`existing_context` / `new_context`）
4. 用户选择后，请求 `POST /api/projects/{project_id}/resolve-conflict-live`。
5. 后端收到选择后，更新运行时术语版本，再继续 section 翻译。

这里更新的是**运行时术语版本**，不是直接写项目术语文件。项目术语文件的持久化管理仍然走术语中心和术语审阅流程。

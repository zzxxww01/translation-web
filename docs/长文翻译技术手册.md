...
**FIRST_ANNOTATE 动态降级：** `_build_glossary_section()` 接受 `term_usage` 参数（由 `TermUsageTracker.used_translations` 提供）。对于 `first_annotate` 策略的术语，如果已在之前的段落中使用过，则不再输出「首次出现时加注释」，改为普通翻译格式。这通过 `build_prompt()` 和 `build_retranslation_prompt()` 的 `term_usage` 参数透传。

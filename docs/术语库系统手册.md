...
2. 运行时候选术语：来自全文分析、prescan、规则提取的临时候选，不一定立刻写入术语库。
...
3. 运行时候选术语
   - 来源：全文分析、section prescan、术语审阅、规则提取。
...
| `strategy` | `TranslationStrategy` | 否 | 翻译策略 | `preserve` / `first_annotate` / `translate` |
...
| 值 | 语义 | 典型表现 |
|---|---|---|
| `preserve` | 保留原文 | `AMD`、`GPU`、`HBM` |
| `first_annotate` | 首次加注，后续可只用译名 | `原文：TSMC；标准写法：台积电；要求：首次出现写“台积电（TSMC）”，后文写“台积电”` |
| `translate` | 直接翻译 | `wafer -> 晶圆` |
...
└── data/
    └── memory/
        └── <project_id>_term_candidates.json
```
...
学习链路从规则中抽出的术语候选会落到：
- `data/memory/<project_id>_term_candidates.json`

这个文件也不是正式术语库，只在术语审阅准备阶段被读取并合并为候选。
...
审阅 payload 中会带 `similar_terms`，帮助用户判断是否与现有术语冲突或重复。
...
统一术语中心组件：
- [web/frontend/src/features/GlossaryFeature.tsx](../web/frontend/src/features/GlossaryFeature.tsx)
...
- 前端入口：[../web/frontend/src/features/GlossaryFeature.tsx](../web/frontend/src/features/GlossaryFeature.tsx)
...
1. `strategy` 会决定是“保留原文”“首次标注”还是“直接翻译”

...
1. **后端发现冲突**：prescan 返回的术语翻译与已有 terminology 冲突时，后端在 SSE 流中发送一个 `term_conflict` 事件，包含冲突术语的原文、已有翻译和 prescan 建议的新翻译。
2. **前端暂停翻译**：前端收到 `term_conflict` 事件后暂停翻译进度，弹出冲突对话框，展示两个选项——保留已有翻译 vs 使用 prescan 建议的新翻译。
3. **用户选择**：用户选择后，前端向后端发送 `POST /api/projects/{project_id}/resolve-conflict-live`，传递用户的决策。
4. **后端存储并继续**：后端收到决策后，将选定的翻译写入术语表，然后继续翻译流程。
...
现在长文翻译的三条业务链路都还在，但运行时只剩新的 `src/prompts/longform/...` 体系；格式处理通过 token 化 + parent block 重建 + 严格导出校验三层保护确保行内样式不被破坏；旧长文 prompt、兼容路由和死代码已经删掉，这份文档里的每个活跃 prompt 都给了真实文件链接，你可以直接从这里进入代码和模板修改项目。

...
确认流的重新翻译面板（`RetranslatePanelEnhanced`）提供三个预设快捷指令，点击后直接以对应 instruction 文本调用重译接口：

| 快捷指令 | ID | 发送给模型的 instruction |
| --- | --- | --- |
| **可读性** | `readable` | 请从句法结构层面优化译文：拆长句（控制在15-30字）、去被动语态改主动、删多余连接词（"此外""然而"）、调整为中文语序、直接用动词不要"对……进行……"、连续三个"的"必须拆句。 |
| **更地道** | `idiomatic` | 请从表达层面优化译文：用具体事实代替空泛描述、用简单结构代替复杂绕行、不要公式化段落和同义词循环、把"基于""鉴于""旨在"换成"根据""考虑到""为了"等自然表达。 |
| **更专业** | `professional` | 请优化译文的专业表达，体现semiAnalysis分析师水准：保留原文观点和判断力度不要弱化、产品/技术代号保留英文（CoWoS、HBM、EUV）行业术语用中文（晶圆代工、良率）金融术语用中文（营收、毛利率）、删除口水话和宣传腔、数据密集段落拆分重组。 |

用户也可以输入自定义指令。所有指令最终都作为 `instruction` 参数传入 `paragraph_retranslate.v2.txt` 模板。

相关代码：
- 快捷指令定义：[web/frontend/src/features/confirmation/components/panels/RetranslatePanelEnhanced.tsx](../web/frontend/src/features/confirmation/components/panels/RetranslatePanelEnhanced.tsx)
...
flowchart TD
    ...
    D1 --> G
    F1 --> G
    G --> H[_extract_term_candidates_from_rules]
    H --> I[data/memory/{project_id}_term_candidates.json]
    I --> J[terminology_review_service._merge_rule_candidates]
```
...
### 9.3 Rules 到术语候选的自动提取

当学习链路保存规则后，`memory_service._extract_term_candidates_from_rules()` 会用正则从规则文本中提取术语翻译对。它识别类似 `"X" 翻译为 "Y"` 的模式，把匹配到的原文术语和中文翻译提取出来，保存到 `data/memory/{project_id}_term_candidates.json`。

这些候选术语随后通过 `terminology_review_service._merge_rule_candidates()` 合并进术语审核流程，使得人工修订中隐含的术语偏好能够自动反哺到后续翻译的术语表中。
...
### 10.3 term_usage 与 first_annotate 策略

`_build_glossary_section()` 现在接受 `term_usage` 参数。当术语表配置为 `first_annotate` 策略时，系统通过 `TermUsageTracker` 跟踪每个术语在译文中是否已经首次出现并被标注过。对于已经标注过的术语，glossary section 中不再插入标注指令，术语仅以纯翻译形式呈现。这使得译文在首次出现时带有注释（如"台积电（TSMC）"），而后续出现时直接使用中文翻译，避免重复标注。

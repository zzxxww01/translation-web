...
3. LLM 配置系统：[src/llm/config_loader.py](../src/llm/config_loader.py)
4. LLM Provider 适配器：[src/llm/provider_adapter.py](../src/llm/provider_adapter.py)
5. 普通段落翻译 agent：[src/agents/translation.py](../src/agents/translation.py)
6. 四步法 agent：[src/agents/four_step_translator.py](../src/agents/four_step_translator.py)
7. 项目级总编排：[src/services/batch_translation_service.py](../src/services/batch_translation_service.py)
8. Prompt 输入 contract：[src/prompts/longform/shared/context_contracts.md](../src/prompts/longform/shared/context_contracts.md)
9. Prompt 输出 contract：[src/prompts/longform/shared/output_contracts.md](../src/prompts/longform/shared/output_contracts.md)

相关阅读：
- [LLM模块系统手册](./LLM模块系统手册.md) - 模型配置、故障转移、Provider 管理
- [长文翻译技术手册](./长文翻译技术手册.md) - 技术细节和术语系统
...
- 当前活跃长文 prompt 一共 14 个（含新增 style_polish），另有 2 份 contract 文档。
- 中文 Markdown 导出新增安全后处理（转义 `$`、CJK-Latin 空格等），由 `src/core/markdown_postprocess.py` 执行。
...
| FourStepTranslator | [src/agents/four_step_translator.py](../src/agents/four_step_translator.py) | prescan / draft / critique / revision / style polish |
...
Phase 0 是长文翻译的**预处理阶段**，在正式翻译前对整篇文档进行全面理解和分析。

**核心设计原则**：
1. **模型选择统一性**：用户选择的模型是整个长文翻译流程的唯一入口，包括Phase 0分析阶段
2. **并发优化**：Phase 0.2a和0.2b独立并发执行，性能提升32-40%
3. **智能采样**：从长文档中智能选取代表性片段，控制在2000字符以内
4. **容错重试**：动态超时重试机制，失败时自动减少采样长度

### 4.1 Phase 0 四阶段流水线

```mermaid
flowchart TD
    A[输入: sections] --> B[Phase 0.1: 全文正则扫描<br/>~0.02秒]
    B --> C{Phase 0.2: 并发执行}
    C --> D[Phase 0.2a: 深度分析文档<br/>~30-60秒]
    C --> E[Phase 0.2b: 验证高频术语<br/>~30-45秒]
    D --> F[术语合并去重]
    E --> F
    F --> G[Phase 0.3: 章节角色分析<br/>~20-40秒]
    G --> H[输出: ArticleAnalysis]
```
...
**实现**：`DeepAnalyzer.extract_high_frequency_terms()`

**扫描规则**：
1. **全大写缩写**（2-6字母）：`\b[A-Z]{2,6}\b` - 如 TSMC, ASIC, SerDes
2. **驼峰命名**：`\b[A-Z][a-z]+(?:[A-Z][a-z]+)+\b` - 如 TeraPHY, PowerBudget
3. **连字符专有名词**：`\b[A-Z][a-z]+-[A-Z][a-z]+(?:-[A-Z][a-z]+)*\b` - 如 Co-Packaged, Multi-Mode
4. **技术术语模式**：`\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+\b` - 如 Silicon Photonics, Optical Engine

**输出**：高频术语列表（频率≥5次，最多30个）+ 术语频率字典

**性能**：~0.02秒（纯正则，无LLM调用）

#### Phase 0.2a - 深度分析文档（30-60秒）

**目的**：理解文档主题、结构、风格，从采样文本中提取术语

**实现**：`LLMProvider.deep_analyze_document()`

**Prompt**：[deep_analyze.txt](../src/prompts/longform/analysis/deep_analyze.txt)
...
```json
{
  ...
  "structure_summary": "文档组织方式和逻辑流",
  "style": {
    "tone": "技术性/正式度",
    "target_audience": "目标读者",
    "translation_voice": "建议的翻译语气"
  },
  "sampled_terms": [
    {
      "term": "术语原文",
      "context_meaning": "在文档中的含义",
      "translation": "建议翻译",
...
**容错机制**：
- 动态重试：采样长度递减（2000→1500→1000→800字符）
- 超时递增：300秒→420秒→600秒
- 4层JSON修复策略：截断、补全、回退、正则提取

#### Phase 0.2b - 验证高频术语（30-45秒）

**目的**：验证Phase 0.1扫描出的高频术语，判断是否为技术术语

**实现**：`LLMProvider.verify_high_frequency_terms()`

**Prompt**：[verify_terms.txt](../src/prompts/longform/analysis/verify_terms.txt)
...
**容错设计**：独立调用，失败不影响整体流程（返回空列表）

**术语合并**：合并Phase 0.2a的sampled_terms和Phase 0.2b的verified_terms，去重后生成最终术语表

#### 并发执行机制

**实现**：使用`ThreadPoolExecutor`并发执行Phase 0.2a和0.2b
...
with ThreadPoolExecutor(max_workers=2) as executor:
    ...
    # 等待两个任务完成
    for future in as_completed([future_deep, future_terms]):
        # 处理结果...
```

**性能提升**：
- 串行执行：60秒 + 45秒 = 105秒
- 并发执行：max(60秒, 45秒) = 60秒
- 提升幅度：42.9%

#### Phase 0.3 - 章节角色分析（20-40秒）

**目的**：分析每个章节在文档中的角色和与前后章节的关系

**实现**：`DeepAnalyzer._analyze_all_section_roles()`

**Prompt**：[section_role_map.txt](../src/prompts/longform/analysis/section_role_map.txt)
...
```json
{
  "section_roles": {
    "section_1": {
      "role_in_article": "章节在全文中的角色",
      "relation_to_previous": "与前一章节的关系",
      "relation_to_next": "与后一章节的关系",
      "key_points": ["要点1", "要点2"],
      "translation_notes": ["翻译注意事项1"]
    }
  }
}
```

**优化**：一次性分析所有章节，减少LLM调用次数

**依赖关系**：必须在Phase 0.2a完成后执行（需要主题和论点信息）

### 4.2 Phase 0 关键优化

#### 1. 拆分调用架构（已解决JSON截断问题）

**问题**：
- 原方案：合并深度分析和术语验证为单次LLM调用
- 响应过长（~25KB）导致JSON截断
- 术语数据丢失（terminology_count: 0）

**解决方案**：
- 拆分为两次独立调用：deep_analyze() + verify_high_freq_terms()
- 减少采样长度：3000 → 2000字符
- 限制高频术语：52 → 30个
- 增强JSON修复：4层策略
...
**原理**：
- Phase 0.2a和0.2b是独立的LLM调用
- 使用ThreadPoolExecutor并发执行
- Phase 0.3依赖0.2a结果，必须串行

**效果**：
- Phase 0.2耗时：105秒 → 60秒（42.9%提升）
- 总耗时：145秒 → 100秒（31.0%提升）
...
**实现**：`SmartSampler` 类（`src/agents/smart_sampler.py`）

**效果**：
- 减少LLM输入长度：17万 → 2000字符
- 保持分析质量：覆盖所有章节
- 降低API成本：减少token消耗
...
**配置**：
```python
ANALYSIS_TIMEOUT_STEPS = (300, 420, 600)  # 秒
ANALYSIS_SAMPLE_STEPS = (2000, 1500, 1000, 800)  # 字符
```

**逻辑**：
1. 第1次尝试：2000字符，300秒超时
2. 第2次尝试：1500字符，420秒超时
3. 第3次尝试：1000字符，600秒超时
4. 第4次尝试：800字符，600秒超时

**触发条件**：超时、连接错误、503/504错误、"temporarily unavailable"

### 4.3 Phase 0 性能指标与模型推荐

基于CPO文章（172,580字符，599段）的完整测试结果：

#### 性能对比

| 模型 | 总耗时 | Phase 0.2耗时 | 术语数 | 转化率 | 推荐度 |
|------|--------|--------------|--------|--------|--------|
| **grok-4-20-non-reasoning** | **53.75s** | 53.59s | 33个 | 63.5% | ⭐⭐⭐⭐⭐ |
| **deepseek-v3** | 114.74s | 114.58s | 22个 | 42.3% | ⭐⭐⭐⭐ |
| **doubao-seed-2-0-lite-260215** | 459.61s | 459.45s | 44个 | 84.6% | ⭐⭐ |

**速度对比**：
- Grok-4比deepseek-v3快2.1倍
- Grok-4比doubao-lite快8.5倍
- Phase 0.1耗时可忽略（~0.02秒）
- 主要耗时在LLM调用（Phase 0.2-0.3占99%以上）

**术语提取对比**：
- **Doubao-lite最全面**：44个术语，转化率84.6%，但存在准确性问题（如FAU翻译错误）
- **Deepseek-v3最保守**：22个术语，转化率42.3%，筛选标准最严格
- **Grok-4平衡**：33个术语，覆盖率和准确性最佳

**质量对比**：
- **论点深度**：Grok-4和Deepseek-v3并列第一（5个论点），Doubao-lite缺失关键战略洞察
- **章节角色分析**：Grok-4最优，能识别"问题—方案—产业生态"论证主线
- **翻译指南**：Grok-4最详细（7条），Deepseek-v3次之（6条），Doubao-lite最少（5条）

#### 最终推荐：grok-4-20-non-reasoning

**核心理由**：
1. **速度压倒性优势**：53.75秒，比deepseek快2.1倍，比doubao快8.5倍
2. **质量不输高端模型**：论点数量、章节分析、翻译指南都是最优或并列最优
3. **术语提取平衡**：33个术语，覆盖率63.5%，准确性高于doubao
4. **成本效益最佳**：速度快、质量高、性价比最优

**详细对比分析**：
- [PHASE0_模型对比分析.md](./PHASE0_模型对比分析.md) - 三模型深度对比（性能、术语、质量）
- [PHASE0_最终推荐.md](./PHASE0_最终推荐.md) - 深度质量评估与推荐理由

#### 并发优化效果

| 指标 | 优化前（串行） | 优化后（并发） | 提升 |
|------|---------------|---------------|------|
| Phase 0.2总耗时 | 105秒 | 60秒 | 42.9% |
| 总耗时 | 145秒 | 100秒 | 31.0% |
| LLM调用次数 | 3次 | 3次 | 无变化 |

**优化原理**：Phase 0.2a（深度分析）和0.2b（术语验证）独立并发执行，使用ThreadPoolExecutor实现。
...
task_defaults:
  ...
  analysis: "grok-4-20-non-reasoning"  # Phase 0默认模型
  title: "deepseek-v3"
```

**用户覆盖机制**：
1. 前端：用户在 `DocumentSidebar.tsx` 选择模型
2. API路由：`translate_projects.py::translate_project_four_step()`
   ```python
   if _body.model:
       from src.api.utils.llm_factory import create_llm_provider
       llm = create_llm_provider(provider=_body.model)
       analysis_llm = llm  # 同时覆盖分析模型
   ```
3. 服务层：`BatchTranslationService` 使用 `analysis_llm`

**关键原则**：
- 用户选择的模型是整个长文翻译流程的唯一入口
- 覆盖所有阶段（包括Phase 0和翻译）
- 严禁硬编码或自动选择其他模型
...
```
API层: translate_projects.py
    ↓
    translate_project_four_step()
    ├─ 创建 BatchTranslationService(llm, analysis_llm)
    └─ 调用 service.translate_project_four_step()
        ↓
服务层: batch_translation_service.py
    ↓
    translate_project_four_step()
    ├─ 创建 DeepAnalyzer(self.analysis_llm)
    └─ 调用 deep_analyzer.analyze(sections)
        ↓
分析层: deep_analyzer.py
    ↓
    analyze(sections)
    ├─ Phase 0.1: extract_high_frequency_terms()
    │   └─ 正则扫描全文
    │
    ├─ Phase 0.2: 并发执行
    │   ├─ ThreadPoolExecutor.submit(
    │   │       llm.deep_analyze_document()
    │   │   )
    │   └─ ThreadPoolExecutor.submit(
    │           llm.verify_high_frequency_terms()
    │       )
    │
    ├─ 合并结果: _parse_combined_analysis_result()
    │
    └─ Phase 0.3: _analyze_all_section_roles()
            └─ llm.analyze_section_roles()
                ↓
LLM层: vectorengine.py / gemini.py
    ↓
    deep_analyze_document()
    ├─ 加载prompt模板: deep_analyze.txt
    ├─ 替换变量: {outline}, {sampled_text}
...
### 5.1 Longform 运行时 Prompt Tree

- Analysis (Phase 0)
  - [deep_analyze.txt](../src/prompts/longform/analysis/deep_analyze.txt) - Phase 0.2a深度分析
  - [verify_terms.txt](../src/prompts/longform/analysis/verify_terms.txt) - Phase 0.2b术语验证
  - [section_role_map.txt](../src/prompts/longform/analysis/section_role_map.txt) - Phase 0.3章节角色
- Terminology
...
- Review
  ...
  - [style_polish.txt](../src/prompts/longform/review/style_polish.txt)
- Auxiliary
...
| --- | --- | --- | --- | --- | --- |
| Analysis (Phase 0) | `longform/analysis/deep_analyze` | [deep_analyze.txt](../src/prompts/longform/analysis/deep_analyze.txt) | [src/llm/base.py](../src/llm/base.py)、[src/agents/deep_analyzer.py](../src/agents/deep_analyzer.py) | Phase 0.2a | 深度分析JSON |
| Analysis (Phase 0) | `longform/analysis/verify_terms` | [verify_terms.txt](../src/prompts/longform/analysis/verify_terms.txt) | [src/llm/base.py](../src/llm/base.py)、[src/agents/deep_analyzer.py](../src/agents/deep_analyzer.py) | Phase 0.2b | 术语验证JSON |
| Analysis (Phase 0) | `longform/analysis/section_role_map` | [section_role_map.txt](../src/prompts/longform/analysis/section_role_map.txt) | [src/agents/deep_analyzer.py](../src/agents/deep_analyzer.py) | Phase 0.3 | 章节角色JSON |
...
| Review | `longform/review/style_polish` | [style_polish.txt](../src/prompts/longform/review/style_polish.txt) | [src/llm/base.py](../src/llm/base.py)、[src/agents/four_step_translator.py](../src/agents/four_step_translator.py) | 四步法（可选 Step 5） | 纯文本 |
...
flowchart TD
    ...
    G --> J2[Style Polish<br/>可选]
    J2 --> J3[style_polish.txt]
...
sequenceDiagram
    ...
    opt conciseness_score < threshold
        FST->>LLM: style_polish(...)
        LLM-->>FST: style_polish
    end
...
| Style Polish（可选） | [four_step_translator.py](../src/agents/four_step_translator.py)、[base.py](../src/llm/base.py) | [style_polish.txt](../src/prompts/longform/review/style_polish.txt) |
...
5. 导出最终阶段新增 Markdown 安全后处理（`markdown_postprocess.py`），转义 `$` 等可能触发渲染器误解的特殊字符，并自动补齐 CJK-Latin 空格。
...
source.html
  ...
  -> Markdown 安全后处理（转义 $、补 CJK-Latin 空格、规范化空行）
```
...
#### 11.6.5 Markdown 安全后处理

在 token 恢复和 block 重建完成后、写入文件前，导出流程会调用 `src/core/markdown_postprocess.py` 的 `postprocess_markdown()` 做最终安全处理：

1. **转义裸 `$`**：防止 MathJax/KaTeX 渲染器误将价格（如 `$100`）当作数学公式。已转义的 `\$` 和成对 `$$` 数学块不受影响。
2. **转义裸 `<` 和行内 `>`**：防止被 Markdown 渲染器误解析为 HTML 标签。保留真正的 HTML 标签和注释。
3. **CJK-Latin 空格**：自动在中文字符与英文字母/数字之间补齐半角空格。
4. **规范化空行**：超过 2 个连续空行压缩为 2 个。
5. **保护区域**：代码块（fenced 和 inline）、图片、链接、HTML 标签/注释在处理期间会被隔离，确保不被误转义。

该后处理已集成到以下导出入口：
- `ProjectManager.export_markdown()`
- `ProjectManager.generate_preview()`
- `MarkdownExporter.export_sections()`
- `MarkdownExporter.export_bilingual()`
...
| 改四步法风格润色（Step 5） | [style_polish.txt](../src/prompts/longform/review/style_polish.txt)、[four_step_translator.py](../src/agents/four_step_translator.py)（`style_polish_threshold`） |
| 改 Markdown 导出后处理规则 | [src/core/markdown_postprocess.py](../src/core/markdown_postprocess.py) |
...
现在长文翻译的三条业务链路都还在，但运行时只剩新的 `src/prompts/longform/...` 体系；四步法新增可选 Style Polish（Step 5），critique 新增 conciseness_score 驱动；翻译 prompt 补强了中文原生表达、隐喻文化适配、跨域术语精度和排版适配四大维度；格式处理升级为”token 化 + 纯链接脱水 + 校验失败自动修复回退 + 严格导出校验 + Markdown 安全后处理”；旧长文 prompt、兼容路由和死代码已经删掉，这份文档里的每个活跃 prompt 都给了真实文件链接，你可以直接从这里进入代码和模板修改项目。

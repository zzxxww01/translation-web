#!/usr/bin/env python3
"""
实时监控翻译进度
"""

import requests
import json
import time
from datetime import datetime

PROJECT_ID = "rl-environments-and-rl-for-science-data-foundries"
API_BASE = "http://127.0.0.1:54321"
MODEL = "deepseek-relay"

def log(msg_type, message):
    timestamp = datetime.now().strftime("%H:%M:%S")
    print(f"[{timestamp}] [{msg_type:8s}] {message}", flush=True)

def monitor():
    url = f"{API_BASE}/api/projects/{PROJECT_ID}/translate-four-step"

    log("INFO", f"开始翻译项目: {PROJECT_ID}")
    log("INFO", f"使用模型: {MODEL}")

    start_time = time.time()
    stats = {
        "total": 0,
        "translated": 0,
        "errors": 0,
        "last_progress": "",
    }

    try:
        response = requests.post(
            url,
            json={"model": MODEL},
            stream=True,
            timeout=7200
        )

        if response.status_code != 200:
            log("ERROR", f"API 错误: {response.status_code}")
            return False

        for line in response.iter_lines():
            if not line:
                continue

            line = line.decode('utf-8')
            if not line.startswith('data: '):
                continue

            try:
                data = json.loads(line[6:])
            except:
                continue

            event_type = data.get('type')

            if event_type == 'start':
                stats['total'] = data.get('total', 0)
                log("INFO", f"总段落数: {stats['total']}")

            elif event_type == 'progress':
                step = data.get('step', '')
                current = data.get('current', 0)
                total = data.get('total', 0)

                # 只显示不同的进度消息
                if step != stats['last_progress']:
                    if total > 0:
                        pct = (current / total) * 100
                        log("PROGRESS", f"{step} ({current}/{total}, {pct:.1f}%)")
                    else:
                        log("PROGRESS", step)
                    stats['last_progress'] = step

            elif event_type == 'error':
                stats['errors'] += 1
                error_msg = data.get('error', '')
                para_id = data.get('paragraph_id', '')

                # 检查 API 余额问题
                if any(kw in error_msg.lower() for kw in ['insufficient', 'quota', 'balance', 'limit']):
                    log("CRITICAL", f"⚠️  API 余额不足！")
                    log("CRITICAL", f"错误: {error_msg}")
                    return False

                log("ERROR", f"段落 {para_id}: {error_msg}")

            elif event_type == 'complete':
                elapsed = time.time() - start_time
                translated = data.get('translated_count', 0)
                total = data.get('total', 0)

                log("SUCCESS", "=" * 60)
                log("SUCCESS", "✓ 翻译完成！")
                log("SUCCESS", f"已翻译: {translated}/{total}")
                log("SUCCESS", f"错误数: {stats['errors']}")
                log("SUCCESS", f"耗时: {elapsed/60:.1f} 分钟")
                log("SUCCESS", "=" * 60)
                return True

            elif event_type == 'incomplete':
                elapsed = time.time() - start_time
                translated = data.get('translated_count', 0)
                total = data.get('total', 0)

                log("WARNING", "=" * 60)
                log("WARNING", "⚠️  翻译未完全完成")
                log("WARNING", f"已翻译: {translated}/{total}")
                log("WARNING", f"错误数: {stats['errors']}")
                log("WARNING", f"耗时: {elapsed/60:.1f} 分钟")
                log("WARNING", "=" * 60)
                return False

            elif event_type == 'cancelled':
                log("WARNING", f"翻译已取消: {data.get('message', '')}")
                return False

            elif event_type == 'heartbeat':
                pass  # 忽略心跳

        log("WARNING", "流结束但未收到完成事件")
        return False

    except KeyboardInterrupt:
        log("INFO", "用户中断")
        return False
    except Exception as e:
        log("ERROR", f"异常: {e}")
        return False

if __name__ == "__main__":
    success = monitor()
    exit(0 if success else 1)

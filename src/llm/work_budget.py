...
@contextmanager
def work_budget_scope(name: str, seconds: float, max_calls: int, *, should_cancel=None):
    from .execution_context import deadline_scope
    budget = WorkBudget(name, seconds, max_calls, should_cancel)
...
def admit_transport():
    # One critical section reserves all scopes, preventing partial reservation
    # and concurrent overspend of the parent run budget.

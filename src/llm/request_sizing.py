...
RequestTooLargeError = RequestBudgetExceeded
...
class RequestBudget:
    max_input: int | None = 24000
    reserved_output: int = 8192
    context_window: int | None = None

    def __post_init__(self):
        for value in (self.max_input, self.reserved_output, self.context_window):
            if value is not None and (type(value) is not int or value <= 0):
                raise ValueError("Request budgets must be positive integers")
    ...
    def check(self, prompt: str, *, output_limit: int | None = None,
        ...
        ceiling = self.max_input if self.max_input is not None else math.inf

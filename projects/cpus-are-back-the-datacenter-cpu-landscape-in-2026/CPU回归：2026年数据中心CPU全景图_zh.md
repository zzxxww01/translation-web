# CPU 回归：2026 年数据中心 CPU 全景图

## 引言

### RL 与 Agent 应用，上下文内存存储，DRAM 价格影响，CPU 互连演进，AMD Venice、Verano、Florence，英特尔（Intel）Diamond Rapids、Coral Rapids，Arm Phoenix + Venom，Graviton 5，Axion

作者：[Gerald Wong](https://substack.com/@geraldwong116502)、[Dylan Patel](https://substack.com/@semianalysis)

2026 年 2 月 9 日 · 付费文章

![](https://substackcdn.com/image/fetch/$s_!Qsru!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F3f9507d8-140b-4db8-9fd6-2ac28050a1ea_2016x1344.png)

自 2023 年以来，数据中心的叙事逻辑极其简单：GPU 与网络设备称王。AI 训练与推理的问世及随后的爆发，抽离了原本属于 CPU 的计算需求。这导致服务器 CPU 的主要供应商英特尔，未能乘上这波数据中心建设与支出狂潮的东风。随着超大规模云厂商与新锐云厂商将重心转向 GPU 及数据中心基础设施，服务器 CPU 营收一直停滞不前。

与此同时，这些超大规模云厂商一直在为其云计算服务自研基于 ARM 架构的数据中心 CPU，直接封死了英特尔一大片可寻址市场。而在自家的 x86 阵地内，英特尔拉垮的执行力，加上面对竞争对手 AMD 时毫无竞争力的性能表现，进一步蚕食了其市场份额。由于拿不出能打的 AI 加速器产品，当整个行业在尽情狂欢时，英特尔只能在原地停滞不前。

过去六个月里，情况发生了翻天覆地的变化。我们在 [Core Research](https://semianalysis.com/core-research/) 与 [Tokenomics Model](https://semianalysis.com/tokenomics-model/) 中发布了多份关于 CPU 需求飙升的报告。我们展示并建模分析的主要驱动力，是强化学习 (RL) 与直觉编程 (vibe coding) 对 CPU 产生的惊人需求。我们还报道了多家供应商与 AI 实验室达成的大型 CPU 云服务协议，并建模分析了正在部署的 CPU 数量及类型。

![](https://substackcdn.com/image/fetch/$s_!5yS5!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F9c9e56be-fcab-4b4a-9a66-0ead1858e8d9_1648x1629.png)

*英特尔 2025 年第四季度数据中心与人工智能事业部 (DCAI) 营收。来源：英特尔*

然而，英特尔近期的强势反弹以及 2025 年下半年需求信号的转变表明，CPU 如今已重回舞台中心。在最新的第四季度财报中，英特尔发现 2025 年底数据中心 CPU 需求出现意外上扬。为此，他们正在上调 2026 年晶圆代工厂（foundry）设备的资本支出（capex）指引，并将晶圆产能分配从 PC 优先向服务器倾斜，以缓解满足这一新增需求时的供应链瓶颈。这标志着 CPU 在数据中心的角色迎来拐点，AI 模型训练与推理对 CPU 的依赖愈发显著。

![](https://substackcdn.com/image/fetch/$s_!AFjW!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F066448fc-f72e-4159-aa67-e0192e2ff2ea_3023x1461.png)

*数据中心 CPU 核心数趋势。来源：SemiAnalysis 预测*

2026 年是数据中心 CPU 令人振奋的一年。在需求激增的背景下，各大供应商都将在今年推出众多新一代产品。因此，本文将为您描绘 2026 年的 CPU 市场格局。我们将首先铺垫背景，回顾数据中心 CPU 的发展史以及不断演变的需求驱动因素，并深入剖析英特尔与 AMD 多年来在数据中心 CPU 架构上的变迁。

随后，我们将目光转向 2026 年的 CPU 产品，全面拆解英特尔的 Clearwater Forest 与 Diamond Rapids 处理器，以及 AMD 的 Venice 处理器。我们将探讨它们在设计上引人注目的趋同（与分歧）之处，分析其性能差异，并预告我们的 CPU 成本分析。

接下来，我们将详细阐述 ARM 阵营的竞争态势。这包括英伟达（NVIDIA）的 Grace 与 Vera 架构、亚马逊的 Graviton 系列、微软的 Cobalt、谷歌的 Axion CPU 系列、Ampere Computing 进军商用 ARM 芯片市场的举措及其被软银收购的事件、ARM 自家的 Phoenix CPU 设计，并审视华为自主研发的 Kunpeng CPU 进展。

面向我们的订阅用户，我们将提供直至 2028 年的数据中心 CPU 路线图，并详细介绍 AMD、英特尔、ARM 与高通（Qualcomm）在 2026 年之后的数据中心 CPU 规划。随后，我们将展望数据中心 CPU 的未来图景，探讨 DRAM 短缺带来的影响，分析英伟达 Bluefield-4 的上下文内存存储平台对通用 CPU 未来的意义，并指出未来 CPU 市场与设计中值得关注的关键趋势。

## 数据中心 CPU 的角色与演进

## PC 时代

![](https://substackcdn.com/image/fetch/$s_!cxhL!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F91d101af-061a-4c19-8b95-81be8c05e26f_2718x1849.png)

*英特尔奔腾 Pro。来源：英特尔*

现代数据中心 CPU 的雏形可追溯至 20 世纪 90 年代。在此前十年间，个人电脑大获成功，让基础计算真正走入千家万户。随着英特尔 i386、i486 和奔腾系列产品的迭代，PC 算力水涨船高。许多原本依赖 DEC 和 IBM 等高级工作站与大型机处理的任务转由 PC 承担，成本仅为前者的零头。面对市场对高性能“大型机替代品”的渴求，英特尔开始推出性能更强、缓存更大的高价位 PC 处理器变体。这一趋势始于 1995 年问世的奔腾 Pro，该处理器采用多芯片模块 (MCM) 技术，将多个 L2 缓存裸片与 CPU 共封装。 至强品牌紧随其后于 1998 年面世，首发产品奔腾 II 至强同样在 CPU 处理器插槽中加入了多个 L2 缓存裸片。尽管时至今日，大型机依然活跃于负责银行交易验证等任务的 IBM Z 系列中，但其已退居小众利基市场，本文不再赘述。

## Dot-Com 时代

21 世纪初迎来了互联网时代。Web 2.0、电子邮件、电子商务、谷歌搜索以及支持 3G 宽带数据的智能手机相继涌现。随着万物触网，数据中心 CPU 必须承担起处理全球互联网流量的重任，并随之成长为一个价值数十亿美元的细分市场。在设计层面，随着登纳德缩放定律走向终结，“主频大战”偃旗息鼓，业界的注意力转向了多核 CPU 与更高的集成度。AMD 将内存控制器直接集成到 CPU 裸片中，高速 I/O（PCIe）也改为直接由 CPU 提供。多核 CPU 格外契合数据中心工作负载，能让大量任务在不同核心上并行处理。

我们将在下文的互连架构章节详细剖析多核连接方式的演进历程。AMD 和英特尔也在这一时期引入了同步多线程 (SMT) 技术。该技术将一个物理核心划分为两个逻辑线程，两者在共享大部分核心资源的同时能够独立运行，从而进一步提升了可并行化数据中心工作负载的性能。对性能有更高要求的用户则会转向多路 CPU 服务器。英特尔的快速通道互联 (QPI) 与 AMD 皓龙 CPU 采用的 HyperTransport 直连架构，能够在单台服务器中为多达八个插槽提供一致性链路。

## 虚拟化与超大规模云计算时代

下一个重大转折点出现在 2000 年代末，即云计算的兴起。这也是整个 2010 年代数据中心 CPU 销量增长的核心驱动力。正如如今 GPU 新锐云厂商的运营模式一样，随着客户将资本支出转化为运营支出（opex），计算资源开始向公有云提供商以及诸如 AWS 等超大规模云服务商集中。受经济大衰退的余波冲击，许多企业已无力购买并维护自有服务器来运行其软件和服务。

云计算提供了一种更易于接受的“按量付费”商业模式。通过租用计算实例并在第三方硬件上运行工作负载，客户支出可随使用量变化而动态调整。这种可扩展性远优于自行采购服务器，因为自有服务器必须时刻保持满负荷运转，才能实现投资回报率 (ROI) 最大化。云技术还催生了更高效便捷的服务，例如 AWS Lambda 等提供的无服务器计算。这类服务能自动为软件分配计算资源，客户在运行特定任务前，无需再费心决定启动多少个实例。由于云厂商在后台包揽了几乎所有工作，计算能力彻底变成了一种商品。

![](https://substackcdn.com/image/fetch/$s_!7ZDt!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F5e31aa5b-fd56-4674-9895-2576e448e40f_1479x1986.png)

*帕特·基辛格（Pat Gelsinger），2012-2021 年任 VMware CEO，2021-2024 年任英特尔 CEO。来源：X @PGelsinger*

构建安全且资源高效的云环境，其核心基石在于 CPU 硬件虚拟化。本质上，虚拟化允许单个 CPU 运行多个独立且安全的虚拟机 (VM) 实例，这些实例由 VMware ESXi 等虚拟机管理程序进行统一调度。多核 CPU 可被灵活划分，将每个虚拟机分配至单个核心或逻辑线程。虚拟机管理程序还能通过网络将实例迁移至不同的核心、插槽或服务器上，以此优化 CPU 利用率，同时确保数据与指令安全，免受同一 CPU 上其他运行实例的干扰。

云计算对虚拟化的需求，叠加 CPU 设计者为提升性能而引入的 SMT 技术，最终在 2018 年被幽灵 (漏洞) 与熔断 (漏洞) 所利用。当两个实例在同一物理核心的不同线程上运行时，攻击者可利用 CPU 核心的分支预测功能，窥探并拼凑出另一线程的数据。分支预测本是一项性能提升技术，通过在程序运行前提前猜测、提取并执行指令，使 CPU 保持满载状态。面对云端安全可能沦陷的危机，各家提供商紧急禁用 SMT，以彻底切断这一攻击路径。 尽管后续推出了补丁与硬件修复方案，但禁用 SMT 导致了高达 30% 的性能损失。这一代价成为英特尔挥之不去的阴影，并直接导致其在后续作出了一系列不合时宜的设计决策，我们将在下文对此展开详述。

## AI GPU 与 CPU 整合时代

新冠疫情引发了一波线上红利，Zoom 视频会议激增、电商繁荣以及网民在线时长的大幅增加，共同推高了互联网流量，数据中心 CPU 的出货量也随之创下历史新高。在 2022 年 11 月 ChatGPT 问世前的五年间，英特尔向云厂商和企业级数据中心累计出货了超 1 亿颗至强可扩展 CPU。

自此以后，AI 模型的训练与推理服务彻底颠覆了 CPU 在数据中心里的地位，并引发了 CPU 部署与设计策略的全面变革。AI 模型计算依赖海量的矩阵乘法，这类运算极易实现并行化，非常适合在 GPU 上大规模执行。GPU 本就配备了庞大的向量单元阵列，这些阵列最初正是为了渲染游戏与可视化 3D 图形而设计的。

尽管加速器节点依然需要宿主 CPU，但 AI 高度结构化且相对简单的计算需求，根本无法发挥 CPU 运行多分支、对延迟敏感代码的优势。此外，CPU 仅有几十个向量单元，而 GPU 动辄数千个，这导致 CPU 的性能与效率落后了 100 到 1000 倍。尤其是当专为 AI 设计的 GPU 引入了专攻矩阵乘法的张量核心 (Tensor Core) 后，差距更是悬殊。尽管英特尔试图通过翻倍的 AVX-512 端口和专用的 AMX (高级矩阵扩展) 加速引擎来强化向量与矩阵运算支持，CPU 依然在数据中心里沦为了配角。然而，互联网服务仍需维持运转，而数据中心的电力却已优先倾斜给 GPU 计算。在双重驱动下，CPU 顺应时代演进，分化成了两大阵营。

### 头节点

头节点 CPU 的职责是管理挂载的 GPU 并持续为其输送数据。为了尽可能压低长尾延迟，系统需要极高的单核性能、大容量缓存，以及高带宽内存与 IO。像英伟达 Grace 这样的专用设计，专门为 GPU 实现了内存一致性访问，使 GPU 能够将 CPU 内存用作模型上下文的键值缓存 (KV Cache) 扩展，这要求 CPU 与 GPU 之间具备极高的带宽。在每个计算节点中，头节点通常采用 1 颗 CPU 搭配 2 颗或 4 颗 GPU 的配置。典型例子包括：

- 每个超级芯片配备 1 颗 Vera CPU 与 2 颗 Rubin GPU

- 每个计算托盘配备 1 颗 Venice CPU 与 4 颗 MI455X GPU

- 每个计算托盘配备 1 颗 Graviton5 CPU 与 4 颗 Trainium3

- 每个节点配备 2 颗 x86 CPU 与 8 颗 TPUv7

### 云原生插槽整合

GPU 疯狂吞噬数据中心功耗预算。为了尽可能高效地处理其余互联网业务，“云原生”CPU 的研发进程全面提速。其核心目标在于：在最优效率（每瓦吞吐量）下，实现单插槽吞吐量与请求处理量的最大化。行业不再一味堆叠新型 CPU 来拉升总吞吐量，而是果断退役低效的老旧服务器，将其替换为数量极少的云原生 CPU。这些新处理器不仅能满足总吞吐量需求，功耗更只有原先的一小部分。这不仅大幅削减了运营成本，更为扩充 GPU 算力腾出了宝贵的功耗预算。

![](https://substackcdn.com/image/fetch/$s_!TfPu!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fbe59d15e-c167-4a38-b904-c6f6e4efa593_2821x1421.png)

*AMD Turin Dense 处理器 7:1 插槽整合。来源：AMD*

业界现已能实现 10:1 甚至更高的插槽整合率。疫情期间云计算投资狂潮中采购的数百万台英特尔 Cascade Lake 服务器正面临退役。取而代之的是 AMD 与英特尔最新一代 CPU，它们在提供同等性能的同时，功耗还不到老架构的五分之一。

架构设计上，这些云原生 CPU 采用面积与能效比高的中型核心，主攻超多核路线。相较于传统 CPU，它们适当削减了缓存容量与 IO 扩展能力。英特尔凭借 Sierra Forest 处理器，将 Atom 核心正式引入数据中心。AMD Bergamo 处理器则采用了面积与能效比更高的 Zen4 核心物理布图。具备出色能效的 ARM 架构设计（如 AWS Graviton 处理器）大获成功；Ampere Computing 也顺势推出 Altra 与 AmpereOne 系列产品线，直指云原生计算市场。

## 强化学习与智能体时代

![](https://substackcdn.com/image/fetch/$s_!Dbd7!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F0ad2959c-94f2-4096-a61c-c40e46ee0dff_3092x949.png)

*微软 Fairwater GPU 与 CPU 数据中心建筑。来源：Google Earth*

如今，为支持头节点之外的 AI 训练与推理，CPU 的使用量再次激增。微软为 OpenAI 建设的 Fairwater 数据中心便是明证。在这里，一栋 48 兆瓦（MW）的 CPU 与存储建筑支撑着 295 兆瓦的主 GPU 集群（GPU cluster）。这意味着现在需要数以万计的 CPU 来处理和管理由 GPU 产生的 PB 级数据，如果没有 AI，这种庞大需求根本无从谈起。

AI 计算范式的演进直接推高了 CPU 的使用强度。在预训练与模型微调阶段，CPU 负责对数据进行存储、分片和索引，随后将其输送至 GPU 集群进行矩阵乘法运算。在多模态模型中，CPU 还被用于图像和视频解码，尽管目前已有越来越多的固定功能媒体加速模块被直接集成到 GPU 中。

![](https://substackcdn.com/image/fetch/$s_!QDm6!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F4658580c-c8cb-4753-b21a-a39831d9a3a8_2052x1554.png)

*强化学习 (RL) 训练循环。RL 环境（绿色部分）中使用的 CPU。来源：SemiAnalysis*

采用强化学习 (RL) 技术来改进模型，进一步推高了 CPU 需求。正如我们在强化学习 (RL) 深度分析中所指出的，在 RL 训练循环中，“RL 环境”需要执行模型生成的动作并计算相应的奖励。在编程和数学等领域，这要求大量 CPU 并行工作，以完成代码编译、验证、解释及工具调用。此外，CPU 还深度参与复杂的物理模拟，并对生成的合成数据进行高精度验证。为进一步扩展模型规模，RL 环境日益复杂。这就要求在主 GPU 集群附近部署大型高性能 CPU 集群，以确保 GPU 满载运行，最大限度减少 GPU 空闲时间。 训练循环对 RL 和 CPU 的依赖愈发显著，正在催生新的瓶颈。由于 AI 加速器的每瓦性能提升速度远超 CPU，这意味着诸如 Rubin 等下一代 GPU，其所需的 CPU 与 GPU 功耗配比，可能会进一步攀升，甚至打破上文 Fairwater 数据中心中 1:6 的比例。

- [扩展强化学习：环境、奖励作弊、智能体、扩展数据](https://newsletter.semianalysis.com/p/scaling-reinforcement-learning-environments-reward-hacking-agents-scaling-data) - Dylan Patel 与 AJ Kourabi · 2025 年 6 月 8 日

在推理端，能够搜索并利用互联网的检索增强生成 (RAG) 模型日益兴起，加之能够调用工具和查询数据库的智能体模型不断涌现，双重驱动下，处理这些请求所需的通用 CPU 算力需求大幅飙升。由于具备向多个数据源发送 API 调用的能力，每个智能体对互联网的使用强度，本质上远超人类进行简单的谷歌搜索。为应对互联网流量的阶梯式跃升，AWS 和 Azure 正在大规模部署其自研的 Graviton 和 Cobalt 系列 CPU，同时采购更多的商用 x86 通用服务器。

步入 2026 年，数据中心对 CPU 和 DRAM 的需求只会愈发强劲。前沿 AI 实验室的 CPU 已经无法满足其 RL 训练需求，正直接与云服务提供商抢夺商用 x86 CPU 服务器，疯狂争夺 CPU 供货配额。面对 CPU 库存意外告罄的局面，英特尔正计划全面上调其至强系列的价格，同时增加生产设备以提振 CPU 产能。AMD 则一直在提升供货能力，旨在实现增长并抢占服务器 CPU 总潜在市场 (TAM) 份额，其笃定该市场在 2026 年将实现“强劲的双位数”增长。接下来，我们将为订阅者深入探讨 2026 年之后 CPU 市场格局的演变。

## 多核 CPU 互联架构演进史

要理解 2026 年 CPU 的设计变革与理念，必须先弄清多核 CPU 的工作原理，以及互连架构随核心数量增长的演进历程。既然有多个核心，自然需要将它们连接起来。2005 年问世的早期双核设计（如英特尔的奔腾 D 和至强 Paxville）仅仅是两个独立的单核。核心间通信必须在封装外部进行，数据通过前端总线 (FSB) 传至北桥芯片，而北桥芯片同时也搭载了内存控制器。 同样在 2005 年推出的 AMD 速龙 64 X2 则算得上是真正的双核处理器。它将两个核心与集成内存控制器 (IMC) 集成在了同一块裸片上。这使得核心之间、核心与内存及 IO 控制器之间，能够通过片上 NoC（片上网络）数据网络直接在芯片内部进行通信。

![](https://substackcdn.com/image/fetch/$s_!BLWt!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F3fc1fbd3-39c1-4650-ab74-54f841f233fc_2329x1801.png)

*英特尔 Tulsa 裸片图。来源：英特尔，Hot Chips 2006*

英特尔随后的 Tulsa 处理器配备了由两个核心共享的 16MB 三级缓存，该缓存同时充当片上的核心间数据网络。后文将会看到，随着核心数量飙升至数百个，这些片上数据网络将成为数据中心 CPU 设计的关键因素。

## Crossbar 架构的局限性

随着设计人员试图进一步增加核心数量，他们触及了这些早期互连架构的扩展瓶颈。由于追求极低延迟与一致性，交叉开关（Crossbar）设计采用了全互连模式，即裸片上的每个核心都有一条独立链路与其他所有核心相连。然而，随着核心数量增多，链路数量激增，导致复杂性大幅上升。

2 个核心：1 条连接

4 个核心：6 条连接

6 个核心：15 条连接

8 个核心：28 条连接

大多数设计的实际极限止步于 4 个核心，核心数更高的处理器则是通过多芯片模块和双核模块来实现的，这些模块在核心对之间共享 L2 缓存与数据网络端口。交叉开关布线通常在共享 L3 缓存上方的金属层中完成，以节省面积。2008 年问世的英特尔 6 核 Dunnington 处理器便采用了三个双核模块，并配备 16MB 共享 L3 缓存。

![](https://substackcdn.com/image/fetch/$s_!6t7R!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F26443b09-cd4f-4635-8189-0a85234e9709_1504x2079.png)

*AMD 皓龙 Istanbul 6 核裸片图。来源：AMD*

AMD 于 2009 年发布 6 核 Istanbul 处理器，配备 6 路交叉开关与 6MB L3 缓存。其 2010 年推出的 12 核 Magny-Cours 采用了两块 6 核裸片，而 16 核 Interlagos 则由两块裸片组成，每块裸片包含四个推土机双核模块。

## 英特尔环形总线

![](https://substackcdn.com/image/fetch/$s_!u8AU!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fe4c3bb6f-9b4b-47c6-9b22-a50aecc28ea0_2424x1789.png)

*英特尔 Nehalem-EX 环形互连架构。来源：英特尔，Hot Chips 2009*

为了突破这一极限，英特尔于 2010 年在其 Nehalem-EX (Beckton) 至强处理器中引入了环形总线架构，将 8 个核心、集成内存控制器以及跨插槽 QPI 链路整合到了单一裸片中。环形总线早年曾应用于 ATi Radeon GPU 与 IBM Cell 处理器中。该架构将所有节点排布成一个环路，环形站点直接集成在 L3 缓存切片内，布线则走缓存上方的金属层。缓存与归属代理负责处理核心间的内存嗅探，并维护与内存控制器的一致性。

来自每个环形站点的核心与 L3 缓存切片的数据经排队后注入环路，数据每个时钟周期向前推进一个站点，直至抵达目标位置。这意味着核心间的访问延迟不再均衡：相比紧邻的核心，位于环路相对两侧的核心需要等待更多周期。为缓解延迟与拥堵，设计中引入了两个反向旋转的环路，并根据地址与环路负载选择最优传输方向。随着布线复杂度得到有效控制，英特尔得以将 Nehalem-EX 的核心数扩展至 8 个，并在 Westmere-EX 上进一步增至 10 个。然而，若继续沿用单环架构进一步扩充核心，过长的环路必将引发一致性与延迟问题。

### Ivy Bridge-EX 虚拟环

![](https://substackcdn.com/image/fetch/$s_!4FIz!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F0830e24f-4740-4e69-8707-090b48c6ec4c_2332x1803.png)

*英特尔 Ivytown 虚拟环。来源：英特尔，Hot Chips 2014*

为了在 Ivy Bridge 世代将核心数扩展至 15 个，英特尔必须在路由拓扑上巧妙布局。核心被排布为 3 列（每列 5 个），并由 3 个“虚拟环”环绕这三列核心。环形站点内的交换机负责控制数据在半环上的传输方向，由此构建出一种“虚拟”的三环架构。

### Haswell 与 Broadwell 双环形总线

![](https://substackcdn.com/image/fetch/$s_!gHb0!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F5b1e5413-09bc-46c0-9cd2-64e233a613ab_2987x1679.png)

*Haswell 高核心数 (HCC) 双环形总线。来源：英特尔*

2014 年，英特尔在 18 核的 Haswell 高核心数 (HCC) 裸片上再次更改了拓扑结构，引入了双独立反向旋转环形总线，两者通过一对双向缓冲交换机相连。内存控制器被分置于两个环形总线中，其中 8 核的环形总线还容纳了 IO 环形站点。中等核心数 (MCC) 裸片变体则将单个半环自身折返相连。2015 年发布的 Broadwell HCC 将核心数提升至 24 个，配备了双 12 核环形总线。

将多个环形总线拼接在一起的缺点在于，核心间通信与内存访问的延迟差异增大，当一个环上的核心访问另一个环的内存时尤为明显。对于那些对延迟敏感且核心间交互频繁的程序而言，这种非均匀内存访问 (NUMA) 对系统性能极为不利。

为了缓解这一问题，英特尔在 BIOS 中提供了一个“片上集群 (CoD)”配置选项，将两个环形总线视为独立的处理器。操作系统会显示 CPU 被拆分为两个 NUMA 节点，每个节点可直接访问一半的本地内存和 L3 缓存。[在 CoD 模式下的测试](https://old.chipsandcheese.com/2023/11/07/core-to-core-latency-data-on-large-systems/)表明，每个环内的延迟保持在 50ns 以下，而跨环访问则超过 100ns，这凸显了经过缓冲交换机所带来的延迟惩罚。

虽然这些方法帮助英特尔将核心数增加到了 24 个，但这既不优雅，也缺乏可扩展性。增加第三个环形总线和另外两组缓冲交换机会过于复杂且不切实际，还会产生过多的 NUMA 集群。要支持更多核心，必须采用全新的互连架构。

## 英特尔 Mesh 架构

![](https://substackcdn.com/image/fetch/$s_!pozD!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F216a402f-a60c-42f3-b7e4-00546356b555_3010x1685.png)

*英特尔 Knights Landing 网格互连架构。来源：英特尔，Hot Chips 2016*

为了解决可扩展性问题，英特尔在 2017 年将其 2016 款至强 Phi “Knights Landing” 处理器中使用的网格互连架构，引入到主流的 Skylake-X 至强可扩展 CPU 中，在 XCC (极高核心数) 裸片中集成了 28 个核心。尽管核心数相比 Broadwell 并未大幅增加，但这一设计奠定了未来十年持续扩展核心数的基础。

在网格架构中，核心呈网格状排列，每行每列通过半环相连，形成 2D 网格阵列。每个网格节点可容纳核心与 L3 缓存切片、PCIe IO、IMC 以及加速器。核心间的路由采用环形机制，数据先沿垂直方向传输，再沿水平方向移动。缓存与主代理及其嗅探过滤器如今分布在所有环形节点上，以维持整个网络内的内存一致性。

在网格网络中，由于多个内存控制器分布在裸片两侧，大型网格的内存访问与核心间延迟差异极大。与早期的片上集群 (CoD) 方法类似，系统提供了几种集群模式，将网格划分为四个象限以实现子 NUMA 集群 (SNC)。这降低了平均延迟，但代价是将每个处理器视为多个插槽，且每个 NUMA 节点的 L3 缓存与内存访问池也相应缩小。

在 Knights Landing 中，每个网格节点容纳两个核心，并共享 L2 缓存。网格阵列规模为 6 列 9 行，顶部和底部行配置了更多的 IO 和 MCDRAM。网格网络运行在独立的时钟频率上，并能动态调整网格时钟以节省功耗。在 Knights Landing 上，网格运行频率为 1.6GHz。

![](https://substackcdn.com/image/fetch/$s_!b8mI!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Ffb55424a-5436-4909-83f4-57f750341ca1_1951x1654.png)

*Skylake-SP 网格架构图。来源：英特尔*

在 Skylake-X 中，28 个核心排列成 6x6 网格阵列，顶部设有北侧 IO 模块，两侧预留了两个 IMC (集成内存控制器) 位置。核心体积增大导致网格阵列规模缩小。为提升浮点性能，这些核心内部增加了更多的 L2 缓存与 AVX-512 扩展。若再增加一行或一列，裸片尺寸必将突破 26 x 33 毫米的光刻掩模版极限。得益于网格规模缩小以及 CPU 频率提升（最高达 4.5GHz），网格时钟频率也随之拉高至 2.4GHz。这使得其平均延迟与 Broadwell 的双环架构不相上下。

随后的 Cascade Lake 与 Cooper Lake 处理器仅作微调，依然沿用相同的 28 核心布局。顺带一提，面对 AMD 携 EPYC 杀回数据中心市场，英特尔在 Cascade Lake-AP 上推出了 56 核心的双裸片 MCM (多芯片模块)，但随后又砍掉了 Cooper Lake CPX-4 的类似版本。

![](https://substackcdn.com/image/fetch/$s_!8h83!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F3aeb2086-3dc2-4707-b47a-1b04ce197224_2176x1604.png)

*Ice Lake XCC (极高核心数) 40 核心网格架构图。来源：英特尔*

下一代 Ice Lake 受益于制程节点从 14nm 演进至 10nm，核心数得以提升至 40 个，呈 8x7 网格排列。这已是光刻掩模版极限下的最大规模。然而，紧随其后的下一代 Sapphire Rapids 仍将停留在同一制程节点，且需集成更多功能特性。这让英特尔在如何继续堆叠核心数上进退维谷。

### 跨 EMIB (嵌入式多芯片互连桥接) 的解耦网格架构

![](https://substackcdn.com/image/fetch/$s_!84cW!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F224c3b3b-2f89-4983-baa8-ba2dfbf79771_2979x1661.png)

*英特尔至强向 Chiplet 演进的解耦之路。来源：英特尔*

![](https://substackcdn.com/image/fetch/$s_!Jaqc!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fcd750739-7f96-4cdb-b968-d5fccfcd99c2_2197x1895.png)

*Sapphire Rapids XCC 拓扑图。来源：英特尔*

Sapphire Rapids 引入了用于矩阵乘法和 AI 的 AMX (高级矩阵扩展) 引擎，核心面积进一步增大。这意味着单片裸片只能容纳 34 个核心，相比 Ice Lake 反而出现了倒退。为了将核心数提升至 60 个，英特尔别无选择，只能再次将核心拆分到多个裸片上。然而，他们希望在芯片层面保持“逻辑上的单片结构”。这样一来，处理器在系统层面的呈现形态和性能表现就能与单片裸片毫无二致。

因此，Sapphire Rapids 首次采用了英特尔的 EMIB (嵌入式多芯片互连桥接) 高级封装技术，实现了跨裸片的网格架构。两对镜像对称的 15 核心裸片通过模块化裸片互连结构拼接在一起。这在四个象限和近 1600 平方毫米的硅片上，构建了一个规模庞大得多的 8x12 网格。为了应对 PCIe 5.0 翻倍的吞吐量与全新数据加速器模块之间激增的数据流量，IO 区域必须配备双排网格节点。

随着跨多裸片的网格规模大幅扩张，核心间的平均延迟从 Skylake 的 47ns 恶化至 59ns。为了尽量避开网格网络，英特尔将每个核心的私有 L2 缓存增加到 2MB。这导致裸片上的 L2 缓存总量甚至超过了 L3 缓存（120MB 对 112.5MB）。此外，官方也更加推荐使用子 NUMA 集群 (SNC) 模式，将每个裸片视为一个独立的象限。

尽管这是英特尔首次转向 Chiplet 架构，但 Sapphire Rapids 却因长达数年的延期和频繁的改版而臭名昭著。或许是由于跨 EMIB 运行网格架构时遇到了性能瓶颈，亦或是出于其他执行层面的问题，其最终版本直到修改至 E5 步进才最终发布。而在最初的路线图中，它的原定发布时间是 2021 年。

2023 年底推出的后续更新版本 Emerald Rapids 沿用了相同的核心架构和制程节点，但将裸片数量减少到了 2 个。得益于 EMIB 裸片间互连占用的硅片面积减少，英特尔得以将核心数从 60 个提升至 66 个（为保证良率，实际最多启用 64 个）。同时，L3 缓存也几乎翻了三倍，达到 320MB。我们曾在此处详细撰文探讨过相关设计决策。

- [英特尔 Emerald Rapids 在 Chiplet 路线上的倒退——设计、性能与成本](https://newsletter.semianalysis.com/p/intel-emerald-rapids-backtracks-on) - Dylan Patel、Gerald Wong 及另外 3 人 · 2023 年 5 月 3 日

### 至强 6 上的异构解耦

![](https://substackcdn.com/image/fetch/$s_!uuve!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fb7e1a665-9bf8-4dd2-873a-4de31bd70c7e_2802x1562.png)

*至强 6 平台特性。来源：英特尔*

![](https://substackcdn.com/image/fetch/$s_!5cM1!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fb4873f4f-99fa-4a9f-9389-b3c1a35f46c6_2510x1047.png)

*至强 6 计算与 I/O 裸片图解。来源：英特尔*

采用多裸片芯粒设计，除了能突破光刻掩模版面积限制外，另一大优势在于能够灵活混搭裸片，并在不同的变体和配置中共享设计。在 2024 年推出的下一代至强 6 平台上，英特尔将 I/O 与核心和内存剥离，走向了异构解耦。此举使得 I/O 裸片得以保留在较旧的 Intel 7 制程节点上，而计算裸片则跃升至 Intel 3 制程节点。因此，英特尔能够复用为 Sapphire Rapids 架构开发的 I/O IP；同时，由于 I/O 模块从先进节点中获益有限，这种做法也有效节约了成本。 与此同时，计算裸片可以与 P 核 Granite Rapids 架构和 E 核 Sierra Forest 架构灵活混搭。在顶配的 Granite Rapids-AP 至强 6900P 系列中，最多可搭载 3 个计算裸片，从而在总计 5 个裸片上构建出一张庞大的 10x19 网格。该网格物理连接了 132 个核心，出于良率考量，实际最多启用 128 个核心。

![](https://substackcdn.com/image/fetch/$s_!Omye!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Ff31bc7da-3964-4e1a-a65c-22d17a7473d4_3102x2196.png)

*至强 6 计算裸片拼接图。从左上角顺时针方向依次为：UCC 44 核、高核心数 (HCC) 50 核、HDCC (高密度核心数) 152 核、LCC 20 核。来源：英特尔，SemiAnalysis 预估*

在 144 核的 Sierra Forest 上，E 核被划分为 4 核集群并共享一个公共网格节点。这些集群排列成 8x6 的网格，裸片上物理光刻了 152 个核心，最多可激活 144 个。尽管 Sierra Forest 是应超大规模云服务商的要求而打造，旨在提供单核 TCO 更低的“云原生”CPU，但英特尔已承认其市场接受度极其有限。目前，这些云厂商早已转向 AMD 并开始自研基于 ARM 的 CPU，而英特尔传统的企业级客户对此也毫无兴趣。结果就是，双裸片 288 核的 Sierra Forest-AP（至强 6900E）SKU 未能实现全面上市，仅作为路线图之外的低产量产品苟延残喘，专供极少数下了订单的超大规模云客户。

### Clearwater Forest 的失败

![](https://substackcdn.com/image/fetch/$s_!-Mf7!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F9e7b64d4-593a-4b8e-a26c-3ddba7850e97_2543x2621.png)

*基于 18A 制程节点的十二个 24 核 Clearwater Forest 计算裸片。来源：英特尔，SemiAnalysis*

这些 I/O 裸片也将被复用于即将推出的至强 6+ Clearwater Forest-AP E 核处理器中。计算裸片首次采用了英特尔的 Foveros Direct 混合键合技术，将 18A 核心裸片堆叠在包含网格、L3 缓存和内存接口的基础裸片之上，使核心总数跃升至 288 个。垂直解耦设计使得计算核心能够向最新的 18A 逻辑制程节点演进，同时将微缩效益不佳的网格、缓存和 I/O 模块保留在较旧的英特尔 3 制程节点上。

![](https://substackcdn.com/image/fetch/$s_!qVHN!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Ff9fc472b-1964-4d8a-9743-0610dd8a10ba_2966x1415.png)

*Clearwater Forest 性能预测。来源：英特尔*

然而，英特尔的执行力问题在 Clearwater Forest 上再次暴露，其上市时间从 2025 年下半年推迟至 2026 年上半年。英特尔将延期归咎于 Foveros Direct 的集成难题。考虑到英特尔在摸索混合键合技术时，竟拿如此复杂的服务器芯片作为首发载体，出现这种情况并不令人意外。或许正因如此，这种垂直解耦的互连架构在访问基础裸片的 L3 缓存和网格网络时，带宽相对较低，每个 4 核集群仅有 35GB/s。

尽管历经两年的时间跨度，并采用了全新的核心微架构、新制程节点、新的先进封装且成本更高，英特尔展示的数据却表明，在同等核心数量下，Clearwater Forest 的性能仅比 Sierra Forest 快 17%。混合键合良率低下导致成本飙升，性能提升却如此有限，难怪英特尔在最新的 2025 年第四季度财报中对 Clearwater Forest 几乎只字未提。我们的判断是，英特尔根本不想大规模量产这些会拖累利润率的芯片，而是宁愿将其作为 Foveros Direct 提升良率的试验载体。

## AMD Zen 互联架构

![](https://substackcdn.com/image/fetch/$s_!sC88!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F7520d55a-ba5b-466e-9799-72fe683a1923_2860x1588.png)

*AMD 霄龙 (EPYC) CPU 演进历程。来源：AMD*

![](https://substackcdn.com/image/fetch/$s_!Z99I!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fc704a28b-2774-4428-9745-2cfdcf1f1573_2775x1508.png)

*英特尔抨击 AMD Naples 处理器。来源：英特尔*

2017 年，AMD 携霄龙 (EPYC) Naples 7001 系列重返数据中心 CPU 市场，在业界引发轩然大波。英特尔嘲讽其设计是“四个胶水拼接的桌面级裸片”，且性能表现参差不齐。现实情况是，AMD 规模不大的设计团队必须精打细算。他们只承担得起流片一款裸片的成本，而这款裸片必须同时兼顾台式 PC、服务器，甚至还要在同一裸片上集成 10Gb 以太网以满足嵌入式需求。

![](https://substackcdn.com/image/fetch/$s_!NxBd!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Ff6de7f59-7300-4154-aba5-836eae048878_3025x1693.png)

*AMD Zeppelin SoC（系统级芯片）架构。来源：AMD，ISSCC 2018*

Naples 采用了四裸片 MCM（多芯片模块）设计，每个“Zeppelin”裸片包含 8 个核心，这使得 AMD 能够以 32 核规格超越英特尔的 28 核。每个裸片容纳 2 个核心复合体 (CCX)，内部的 4 个核心与 8MB L3 缓存通过交叉开关相连。裸片上的可扩展数据网络实现了 CCX 间的通信。封装内 Infinity Fabric (IFOP) 链路将每个裸片与封装内的其他 3 个裸片相连，而插槽间 Infinity Fabric (IFIS) 链路则用于实现双插槽设计。Infinity Fabric 实现了裸片间的一致性内存共享，该技术源自 AMD 早期的 HyperTransport 技术。

这种架构意味着没有统一的 L3 缓存，且核心间延迟差异巨大。信号从一个裸片上 CCX 内的核心传输到另一个裸片上的核心，需要经过多次跳跃。一台典型的双插槽服务器最终会存在四个 NUMA 域。其通信层级包括：CCX 内部、CCX 之间、MCM 裸片之间、插槽之间。性能表现直接反映了这一点：渲染等高度可并行、极少需要核心间通信和内存访问的任务表现优异；而更依赖核心间通信、对内存和延迟敏感的任务则表现糟糕。此外，由于当时大多数软件并未针对 NUMA 架构进行优化，这让英特尔对 AMD“性能表现不稳定”的抨击显得确有其事。

### 霄龙 (EPYC) Rome 的集中式 I/O

![](https://substackcdn.com/image/fetch/$s_!qr8U!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F89034fe0-5221-40fe-9936-7e7e779456d6_2830x1602.png)

*Rome 与 Milan SoC 架构。来源：AMD*

2019 年发布的 Rome 处理器彻底重构了裸片布局。AMD 借助异构解耦打造出 64 核产品，将当时仍停留在 28 核的英特尔远远甩在身后。八个 8 核核心计算裸片（CCD）环绕着一个中央 I/O 裸片，后者集成了内存和 PCIe 接口。CCD 升级至最新的台积电 N7 工艺，而 I/O 裸片则继续沿用格芯的 12nm 工艺。CCD 依然由两个 4 核 CCX 组成，但它们之间不再直接通信。相反，所有跨 CCX 的数据流量均通过 I/O 裸片路由，信号通过全局内存互连 (GMI) 链路在基板上传输。这意味着 Rome 在功能上表现为 16 个 4 核 NUMA 节点，但仅有两个 NUMA 域。

与上一代 Naples 类似，在 Rome 上启动的虚拟机（VM）必须限制在 4 核以内，以免因跨裸片通信造成性能损耗。2021 年发布的 Milan 处理器解决了这一问题。它转向了环形总线架构，将 CCX 规模扩大至 8 核，同时继续复用与 Rome 相同的 I/O 裸片。

![](https://substackcdn.com/image/fetch/$s_!yJpR!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F053b0db0-b10d-46be-ab7c-6826eeb1b607_2777x1154.png)

*AMD Turin-Dense 架构。来源：AMD*

尽管最初计划采用先进封装，AMD 在接下来的两代产品中依然坚守了这一熟悉的架构设计。2022 年发布的 Genoa 处理器扩展至 12 个 CCD，而 2024 年发布的 Turin 处理器（如 128 核的霄龙 (EPYC) 9755）更是搭载了多达 16 个 CCD。这些 CCD 均围绕一个中央 I/O 裸片排列，该 I/O 裸片已升级支持 DDR5 和 PCIe 5.0 接口。

这种芯粒设计的核心优势在于，仅需一次裸片流片即可实现核心数量的可扩展性。AMD 只需设计单一 CCD，通过组合不同数量的 CCD，就能在整个 SKU 矩阵中提供全系列的核心数量配置。此外，每个 CCD 的裸片面积较小，这不仅有助于提升良率，还能在向新制程节点迁移时加快上市时间。这与网格（mesh）架构设计形成鲜明对比：后者采用接近光刻掩模版极限尺寸的大型裸片，且针对不同核心数量的较小网格配置，必须进行多次流片。在共享同一 I/O 裸片和插槽平台的前提下，AMD 还能灵活替换不同的 CCD 设计。例如，AMD 借助紧凑的 Zen 4c 核心打造了 Bergamo 处理器，并利用 Zen 5c 核心推出了 192 核的 Turin 衍生版本。 我们此前曾撰文分析过这种专为高效云计算打造的新型核心衍生版本。架构解耦还允许打造更小规模的版本，例如 EPYC 8004 Siena 处理器，该产品在 6 通道内存平台上仅使用了 4 个 Zen 4c CCD。

- [Zen 4c：AMD 对超大规模 ARM 与英特尔 Atom 的有力回击](https://newsletter.semianalysis.com/p/zen-4c-amds-response-to-hyperscale) - Dylan Patel 与 Gerald Wong · 2023 年 6 月 5 日

## 英特尔 Diamond Rapids 架构变革

![](https://substackcdn.com/image/fetch/$s_!ZcsD!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F73fc256f-03cf-47b4-9ac5-a6240b0c9de0_2786x1606.png)

*Diamond Rapids 架构概览。来源：HEPiX，经由 @InstLatX64*

乍看之下，Diamond Rapids 简直是 AMD 设计的翻版，计算裸片紧紧环绕着中央的 I/O 裸片。看来要在 Granite Rapids 的 10x19 规模之上继续扩展单一网格网络以增加核心数，难度实在太大。这意味着英特尔最终妥协，接受了多 NUMA 节点与多 L3 缓存域的架构。四个核心构建块 (CBB) 裸片分布在两侧，将两个 I/O 与内存集线器 (IMH) 裸片夹在中间。

在每个 CBB 内部，32 个基于 Intel 18A-P 工艺的双核模块 (DCM) 被混合键合到基础的 Intel 3-PT 裸片上，该基础裸片包含 L3 缓存与本地网格互连。为了减少网格站点数量并降低网络流量，现在每个 DCM 中的两个核心共享一个 L2 缓存。这种设计让人不禁联想到 2008 年的 Dunnington 架构。虽然这意味着 Diamond Rapids 总共拥有 256 个核心，但主流 SKU 似乎最多只会启用 192 个核心。受限于较低良率，更高核心数的版本大概率将预留给路线图之外的定制订单。

IMH 裸片包含 16 通道 DDR5 内存接口、支持 CXL3 的 PCIe6，以及英特尔数据路径加速器（QAT、DLB、IAA、DSA）。

有趣的是，裸片间互连似乎不再需要 EMIB (嵌入式多芯片互连桥接) 先进封装。封装基板上的长走线将每个 CBB 裸片与两个 IMH 裸片直接相连。这使得每个 CBB 都能直接访问完整的内存与 I/O 接口，无需再额外跳转至另一个 IMH。这也确保了任何 CBB 间的通信仅需 2 次跨裸片跳数。由于放弃了先进封装并将核心拆分到 4 个裸片上，我们预计跨 CBB 的延迟将明显恶化，与同一裸片内的延迟相比将出现巨大落差。

![](https://substackcdn.com/image/fetch/$s_!v88S!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fe49ef13f-5a91-465a-af7e-3caabc9c651a_2942x1627.png)

*英特尔在其 P 核 (性能核) 上移除了 SMT。来源：英特尔*

延迟恶化固然棘手，但 Diamond Rapids 最致命的问题在于砍掉了 SMT。幽灵 (漏洞) 和熔断 (漏洞) 对英特尔的根本性打击远超 AMD，英特尔核心设计团队对此心有余悸，因此从 2024 年客户端 PC 的 Lion Cove 架构开始，他们在设计 P 核 (性能核) 时便彻底抛弃了该功能。英特尔当时给出的合理化解释是，移除 SMT 功能所节省的芯片面积能带来更好的效率，代价则是牺牲原始吞吐量。这对 PC 设计来说并无大碍，因为它们同时集成了 E 核 (能效核)，足以提振多线程性能。

然而，最大吞吐量对数据中心 CPU 至关重要，这让 Diamond Rapids 陷入了严重劣势。与当前 128 核、256 线程的 Granite Rapids 相比，我们预计主流的 192 核、192 线程 Diamond Rapids 速度仅能提升约 40%，这将导致英特尔在又一代产品中再次面临性能落后于 AMD 的窘境。

在产品开发后期，英特尔彻底砍掉了主流的 8 通道 Diamond Rapids-SP 平台，导致其出货量最大的核心市场至少要到 2028 年才会迎来换代产品。尽管这有助于精简英特尔臃肿的 SKU 产品线，但我们认为这是一步错棋，因为用于 AI 工具调用和上下文存储的通用计算，更多依赖于具备良好连接性的主流 CPU，而非追求极致单插槽性能的产品。

## AMD Venice 架构变革

![](https://substackcdn.com/image/fetch/$s_!hYF-!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F5ccdb80a-accb-4092-90d4-09ebda6b6953_1530x1600.png)

*AMD Venice 裸片布局。来源：@HighYieldYT*

尽管英特尔放弃了 EMIB (嵌入式多芯片互连桥接)，AMD 却终于在 Venice 处理器上采用了同等的高级封装技术，通过高速短距链路将 CCD 与 I/O 裸片相连。我们在[加速器、HBM 与高级封装模型](https://semianalysis.com/accelerator-hbm-model/)中提供了相关的出货量数据。

CCD 链路所需的额外边缘布线空间 (Shoreline) 占据了更多宽度，迫使中央 I/O 枢纽必须拆分为两块裸片。这导致在芯片的两半之间通信时，会增加一次裸片间跳转，进而形成了一个英特尔方案所能避免的新 NUMA 域。目前，I/O 裸片总共配备 16 个内存通道，多于 2022 年 Genoa 架构的 12 个。AMD 也终于追平英特尔，开始支持多路复用内存以获取更高带宽；其 16 通道多路复用双列直插式内存模块 (MRDIMM)-12800 可提供 1.64TB/s 的带宽，达到 Turin 的 2.67 倍。

AMD 还在 CCD 内部转向了 Mesh 网络，采用 4x8 网格布局容纳 32 个 Zen6c 核心，不过可能还会包含一个额外的备用核心以提升良率。8 个台积电 N2 工艺的 CCD 将总核心数提升至 256 个，相比 192 核的 Turin-Dense 3nm 霄龙 (EPYC) 9965 增加了三分之一。Zen6c 为每个核心配备了完整的 4MB L3 缓存 (此前在 Zen5c 上该容量被减半)，从而在每个 CCD 上形成了 128MB 的大容量缓存区。

针对 AI 头节点优化、核心数较少且主频更高的“-F”系列 SKU，将采用与其消费级桌面和移动 PC 产品线相同的 12 核 Zen6 CCD 设计，通过 8 个 CCD 实现最多 96 个核心。尽管相比 128 核的 Turin-Classic 4nm 霄龙 (EPYC) 9755 出现倒退，但与高频版的 64 核 EPYC 9575F 相比，其核心数依然增加了 50%。

最后，在 I/O 裸片旁边、DDR5 接口引出处，可以看到 8 个小裸片。这些是集成无源器件 (IPD)，有助于在 I/O 极度密集的区域为芯片平稳供电；在该区域，SP7 (插槽) 的封装布线已被内存通道的扇出完全占满。

![](https://substackcdn.com/image/fetch/$s_!0aVL!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fe982a29b-8dbe-48a8-b2d8-a5a595d09ffe_3053x1668.png)

*AMD Venice 性能声明。来源：AMD*

在性能方面，AMD 声称在 SPECrate®2017_*int*_base 测试中，顶配 256 核版本的每瓦性能比顶配 192 核 Turin 高出 1.7 倍以上。得益于具备更高每时钟周期指令数 (IPC) 的全新 Zen 6 核心微架构，这意味着其单核性能实现了进一步跃升。Zen 6 还针对 AI 数据类型引入了新指令，包括 AVX-512_*FP16、AVX_*VNNI_*INT8，以及一种全新的 AVX-512_*BMM 指令，用于在 CPU 浮点单元上执行位矩阵乘法和位反转操作。

对于 BMM 指令，FPU 寄存器会存储 16x16 二进制矩阵，并利用 OR（或）与 XOR（异或）运算来计算 BMM 累加结果。二进制矩阵的计算难度远低于浮点矩阵。对于能够利用该特性的软件（如 Verilog 仿真），它能带来大幅效率提升。然而，BMM 缺乏大语言模型 (LLM) 所需的足够精度，因此我们认为该指令的实际采用率将非常有限。

鉴于 AMD 的单核性能已显著领先于英特尔（96 核 Turin 可匹敌 128 核 Granite Rapids），在 2026 至 2028 年这一代数据中心 CPU 中，AMD Venice 与 Intel Diamond Rapids 之间的性能差距将进一步拉大。得益于全新的裸片间互连技术和更大的核心域，Venice 的核心间延迟预计将优于 Turin。

在英特尔抽身退出的领域，AMD 正在持续加码。英特尔直接砍掉了其 8 通道处理器，而 AMD 则将推出全新的 8 通道 Venice SP8 平台。该平台将作为低功耗、小插槽 霄龙 (EPYC) 8004 Siena 系列的继任者，同时仍能提供多达 128 个高密度 Zen 6c 核心。借此，AMD 必将在企业级市场这一英特尔的传统腹地中，斩获大量市场份额。

## 2026 年 CPU 成本分析

![](https://substackcdn.com/image/fetch/$s_!hsEy!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F8c0105ff-d533-4e0b-b724-ed097370cf6f_1065x1901.png)

*AMD Venice 物料清单（BoM）成本分析。来源：SemiAnalysis 估算 sales@semianalysis.com*

*SemiAnalysis 凭借对供应链的深入了解，提供详尽的物料清单成本分析。若需了解确切的裸片尺寸、配置、拓扑结构、性能估算，以及与超大规模云厂商 ARM CPU 的竞争力对比，请通过 [Sales@SemiAnalysis.com](mailto:Sales@SemiAnalysis.com) 联系我们，获取定制化咨询与竞品分析服务。我们掌握 AMD Turin、Venice，英特尔 Granite Rapids、Diamond Rapids，英伟达 Grace、Vera，以及来自 AWS、微软、谷歌等超大规模云厂商 ARM CPU 的详细成本与拆解数据。*

## 英伟达 Grace

![](https://substackcdn.com/image/fetch/$s_!Fg1r!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F56be8f6f-b46d-4a8c-b042-449a59a8fe0c_1999x1018.png)

*英伟达 Grace CPU 连接示意图。来源：英伟达*

![](https://substackcdn.com/image/fetch/$s_!oe6W!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F5e6f2555-0038-48f1-945e-f48bdc05c2f7_1846x1046.png)

*英伟达 Grace 可扩展一致性网络。来源：英伟达*

与本文讨论的大多数通用 CPU 不同，英伟达的 CPU 在设计之初就充分考虑了头节点（head nodes）和扩展 GPU 内存的需求，并将 NVLink-C2C 作为其杀手锏。这条 900GB/s（双向）的高速链路允许相连的 Hopper 或 Blackwell GPU 全带宽访问 CPU 内存。每颗 Grace CPU 内存容量高达 480GB，有效缓解了 HBM 内存容量不足的限制。Grace 还采用了移动级 LPDDR5X 内存，在 512 位宽内存总线上维持 500GB/s 高带宽的同时，有效降低了非 GPU 功耗。初代 Grace Hopper 超级芯片为每颗 GPU 搭配 1 颗 Grace，而后续的 Grace Blackwell 世代则让 2 颗 GPU 共享 1 颗 CPU。此外，英伟达还为需要高内存带宽的 HPC 客户提供了双 Grace 超级芯片 CPU。

在 CPU 核心方面，英伟达采用了高性能的 ARM Neoverse V2 设计，每核心配备 1MB 私有 L2 缓存。其 6x7 网格网络容纳了 76 个核心和 117MB 的 L3 缓存，为保证良率，最多启用 72 个核心。网格节点上的每个缓存交换节点（CSN）最多连接 2 个核心和 L3 缓存切片。英伟达着重强调了该网格网络高达 3.2TB/s 的对分带宽，这表明 Grace 专门侧重于数据流，而非单纯的 CPU 原始性能。

在性能方面，Grace 存在一个源自 Neoverse V2 核心的古怪微架构瓶颈，导致其运行未经优化的 HPC 代码时速度缓慢。根据英伟达的 [Grace 性能调优指南](https://docs.nvidia.com/dccpu/grace-perf-tuning-guide/compilers.html)，针对更好的代码局部性来优化大型应用，可带来 50% 的提速。这是因为核心分支预测引擎在提前存储和获取指令方面存在局限。在 Grace 上，指令被组织成 32 个 2MB 的虚拟地址空间。

当这个分支目标缓冲区填满超过 24 个区域时，性能开始一路陡降。热点代码会霸占缓冲区并加剧指令搅动（instruction churn），从而导致更多的分支预测失误。如果程序超出 32 个区域，整个 64MB 缓冲区将被清空，分支预测器会遗忘所有先前的分支指令，以腾出空间容纳新指令。在失去有效运作的分支预测器后，CPU 核心的前端会成为整个操作的瓶颈，导致 ALU 只能闲置等待指令执行。

正因如此，目前在 GB200 和 GB300 中，AI 工作负载会被 Grace CPU 拖慢。

### 英伟达 Vera

2026 年，Vera 将在 Rubin 平台上更进一步。其 C2C 带宽翻倍至 1.8TB/s；内存位宽同样翻倍，配备 8 个 128 位宽的 SOCAMM 192GB 模块，提供 1.5TB 内存容量和 1.2TB/s 带宽。网格网络设计得以保留，7x13 的网格容纳了 91 个核心，其中最多激活 88 个。L3 缓存增加至 162MB。英伟达现将外围内存和 I/O 区域解耦为独立的芯粒（chiplet），共计 6 颗裸片采用 CoWoS-R 封装（包括 1 颗采用 3nm 工艺并配备 NVLink-C2C 的光罩极限尺寸（Reticle-sized）计算裸片、4 颗 LPDDR5 内存裸片以及 1 颗 PCIe6/CXL3 IO 裸片）。

![](https://substackcdn.com/image/fetch/$s_!C8_g!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F36a84fe6-b848-4374-9c7f-245cc317e0a3_1989x1851.png)

*Vera Rubin NVLink C2C 架构图。来源：英伟达*

![](https://substackcdn.com/image/fetch/$s_!YHta!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Febce2cd3-75fb-44fe-aa0b-35a191131a98_3119x1925.png)

*Vera CPU 规格。来源：英伟达*

![](https://substackcdn.com/image/fetch/$s_!CdZY!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F24ed62c6-9b02-438e-8acb-1868bfd4ee81_3000x3040.jpeg)

*Vera 物理布图注释版。来源：英伟达，SemiAnalysis 测算*

或许是吃尽了 ARM Neoverse 内核性能瓶颈的苦头，英伟达重新召回了其定制化 ARM 内核设计团队，推出了支持 SMT 的全新 Olympus 内核，实现了 88 核 176 线程的配置。英伟达上一款定制内核还要追溯到 8 年前，即搭载 10 宽发射 Carmel 内核的 Tegra Xavier SoC。基于 ARMv9.2 架构的 Olympus 内核将浮点单元宽度从 Neoverse V2 的 4 个端口提升至 6 个 128b 宽端口，现已支持 ARM 的 SVE2 FP8 运算。每个内核配备 2MB 私有 L2 缓存，容量较 Grace 翻倍。综合来看，英伟达宣称升级至 Vera 后整体性能实现了 2 倍提升。

## AWS Graviton5

![](https://substackcdn.com/image/fetch/$s_!wrPN!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Faf081f84-4b44-4861-83b9-7467a1b74f89_2964x1485.png)

*Graviton CPU 发展史。来源：AWS*

亚马逊云科技 (AWS) 是首家成功为云端开发并部署自研 CPU 的超大规模云厂商。得益于收购 Annapurna Labs 芯片设计团队，并采用 ARM 的 Neoverse 计算子系统 (CSS) 参考设计，AWS 如今能以更低价格提供 EC2 云实例。他们跳过采购英特尔至强 (Xeon) 处理器，直接交由台积电和外包半导体封装测试 (OSAT) 合作伙伴生产芯片；这种模式大幅改善了利润率结构，为降价提供了空间。

Graviton 的全面发力始于疫情繁荣期的 Graviton2 代产品。当时 AWS 抛出大幅折扣，吸引云客户将程序从 x86 架构迁移至 ARM 生态。尽管单核性能不及英特尔同期的 Cascade Lake，但 Graviton2 以极低的价格提供了 64 个 Neoverse N1 核心，性价比实现了大幅跃升。

2021 年底亮相的 Graviton3 预览版引入了多项变革，核心目标是将单核性能提升至具备竞争力的水平。AWS 转向了 ARM 的 Neoverse V1 架构；这是一款面积更大的 CPU 核心，浮点性能达到 N1 的两倍，而总核心数仍维持在 64 个。该芯片采用 10x7 核心网格网络 (CMN)，裸片上实际物理布图包含 65 个核心，预留 1 个核心以便在芯片分级 (Binning) 时屏蔽。AWS 还采用了小芯片 (Chiplet) 解耦设计：台积电 N5 工艺制造的中央计算裸片周围，环绕着 4 个 DDR5 内存小芯片和 2 个 PCIe5 I/O 小芯片，所有模块均通过英特尔的嵌入式多芯片互连桥接 (EMIB) 先进封装技术互连。受英特尔 Sapphire Rapids 延期影响，Graviton3 顺势成为首批部署 DDR5 和 PCIe5 的数据中心 CPU，比 AMD 和英特尔整整领先了一年，我们曾在此前文章中对此做过详细分析。

- [亚马逊 Graviton 3 采用小芯片与先进封装推动高性能 CPU 商品化 | 首款 PCIe 5.0 与 DDR5 服务器 CPU](https://newsletter.semianalysis.com/p/amazon-graviton-3-uses-chiplets-and) - Dylan Patel · 2021 年 12 月 2 日

Graviton4 延续了规模扩张的步伐，采用升级版 Neoverse V2 核心，将核心数和内存通道数均提升 50%，分别达到 96 核与 12 通道，较上一代实现了 30% 至 45% 的性能提速。PCIe5 通道数从 32 条激增至三倍达到 96 条，大幅增强了与网络及存储设备的连接能力。Graviton4 还引入了对双路插槽配置的支持，进一步推高了单实例的核心上限。

![](https://substackcdn.com/image/fetch/$s_!P_3k!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fcaa991f9-af71-4c1d-b519-c7aa45b5bfac_2732x1472.png)

*Graviton5 核心架构图。来源：AWS*

自 2025 年 12 月推出预览版以来，Graviton5 的性能再次实现大幅跃升。它采用台积电 3nm 工艺，集成 1720 亿个晶体管，搭载 192 个 Neoverse V3 核心，较上一代翻倍。虽然每核二级缓存维持在 2MB，但共享三级缓存从 Graviton4 少得可怜的 36MB 增至 Graviton5 相当可观的 192MB。尽管核心数翻倍，内存带宽却仅提升了 57%（12 通道 DDR5-8800），这些额外增加的缓存正好充当了缓冲池。

正如我们在 [Core Research](https://semianalysis.com/core-research/) 中所探讨的，Graviton5 的封装非常独特，这对供应链中的几家供应商产生了深远影响。

有趣的是，虽然 PCIe 通道升级到了 Gen6，但通道数量却从 Graviton4 的 96 条倒退回 Graviton5 的 64 条，显然是因为 AWS 在实际部署中通常用不满所有 PCIe 通道。这种成本优化在不影响性能的前提下，为亚马逊节省了大量 TCO。

Graviton5 采用了演进版的芯粒架构与互连技术，现在每 2 个核心共享同一个网格节点，整体呈 8x12 网格排列。虽然 AWS 这次并未展示具体的封装和裸片配置，但他们确认 Graviton5 确实采用了全新的封装策略，并且 CPU 核心网格被拆分到了多个计算裸片上。

![](https://substackcdn.com/image/fetch/$s_!0qb4!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F9280a965-5af9-4c30-8ae0-b107f9248e48_2697x1149.png)

*Graviton 硅前设计。来源：AWS*

在 CPU 使用方面，AWS 自豪地提到，他们已在内部持续集成/持续部署 (CI/CD) 设计流程中使用了数千颗 Graviton CPU，并运行电子设计自动化 (EDA) 工具来设计和验证未来的 Graviton、Trainium 和 Nitro 芯片，由此形成了一个“用 Graviton 设计 Graviton”的内部试用 (Dogfooding) 闭环。AWS 还宣布，其 Trainium3 加速器将采用 Graviton CPU 作为头节点，比例为 1 颗 CPU 搭配 4 颗 XPU。虽然初始版本搭载的是 Graviton4，但未来的 Trainium3 集群将由 Graviton5 驱动。

## 微软 Cobalt 200

![](https://substackcdn.com/image/fetch/$s_!VLAl!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F9634fe8d-37d6-4a92-87a5-1b371d9a6a4f_1920x1080.png)

*Microsoft Cobalt 200 服务器。来源：Microsoft*

![](https://substackcdn.com/image/fetch/$s_!nG1L!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F80839a8f-c1e0-44fe-ab4e-310b4427ccd1_2608x1427.png)

*Cobalt 200 SoC 物理布图。来源：Microsoft*

- [Microsoft 基础设施——AI 与 CPU 定制芯片 Maia 100、Athena、Cobalt 100](https://newsletter.semianalysis.com/p/microsoft-infrastructure-ai-and-cpu) - Dylan Patel 与 Myron Xie · 2023 年 11 月 15 日

继我们在前文探讨过的 2023 年微软首款 Cobalt 100 CPU 之后，Cobalt 200 于 2025 年底发布，并带来了多项升级。虽然核心数量增幅不大（从 128 个微增至 132 个），但与上一代的 Neoverse N2 相比，采用 Neoverse V3 设计的新核心性能大幅提升。每个核心配备高达 3MB 的超大 L2 缓存。这些核心分布在两颗台积电 3nm 计算裸片上，通过标准的 ARM Neoverse CMN S3 网格网络连接，裸片之间则采用定制的高带宽互连技术。从架构图来看，每颗裸片采用 8x8 网格布局，配备 6 个 DDR5 通道以及 64 条支持 CXL 的 PCIe6 通道。每 2 个核心共享一个网格节点（mesh stop）。每颗裸片上物理集成了 72 个核心，出于良率考量，实际启用了 66 个。此外，网格网络中还分布着 192MB 的共享 L3 缓存。 凭借这些升级，Cobalt 200 的运行速度较 Cobalt 100 提升了 50%。

与 Graviton5 不同，Cobalt 200 将仅用于 Azure 的通用 CPU 计算服务，不会被用作 AI 头节点。微软的 Maia 200 机架级系统转而部署了英特尔的 Granite Rapids CPU。

## 谷歌 Axion C4A 与 N4A

![](https://substackcdn.com/image/fetch/$s_!iUhJ!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F05f65e22-af68-4a66-8471-20eb13de627b_3005x1594.png)

*Axion C4A 晶圆与封装。来源：Hajime Oguri, Google Cloud Next ’24*

![](https://substackcdn.com/image/fetch/$s_!8nFB!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F7cca6b00-0503-42dd-b09d-15a595e864d9_1844x1814.png)

*Axion N4A CPU。来源：Google*

Axion 产品线于 2024 年发布，并于 2025 年全面上市，标志着谷歌正式为其 GCP (谷歌云平台) 云服务引入定制 CPU 芯片。Axion C4A 实例采用大型单片 5nm 裸片，在标准网格网络上搭载多达 72 个 Neoverse V2 核心，并配备 8 通道 DDR5 与 PCIe5 连接。根据 Google Cloud Next 2024 大会展示的 Axion 晶圆特写图，该裸片似乎以 9x9 网格布局光刻了 81 个核心，预留了 9 个核心供屏蔽以保障良率。因此我们笃定，2025 年底开启预览的 96 核 C4A 裸金属实例必然采用了全新设计的 3nm 裸片。

针对更具成本效益的横向扩展 Web 与微服务，谷歌目前已推出 Axion N4A 实例预览版。该实例在尺寸大幅缩小的裸片上搭载了 64 个性能较低的 Neoverse N3 核心，为 2026 年全年的大规模产能爬坡铺平了道路。Axion N4A 芯片由谷歌基于台积电 3nm 工艺全定制设计。随着谷歌将内部基础设施向 ARM 架构迁移，Gmail、YouTube、Google Play 等服务将在 Axion 与原有的 x86 架构上并行运行。未来，谷歌将专门设计 Axion CPU，用作驱动 Gemini 的 TPU (张量处理器) 集群的头节点。

## AmpereOne 与软银收购案

![](https://substackcdn.com/image/fetch/$s_!2wN-!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fae88cb59-dfc9-4ef0-82db-daeb13090a11_2774x1467.png)

*AmpereOne 2024 年路线图。来源：Ampere Computing*

![](https://substackcdn.com/image/fetch/$s_!tg4g!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F9bd4b346-265c-417c-8de2-3bb02f84db2d_2618x1683.png)

*Ampere Altra Max（左）与 Altra（右）。来源：Ampere Computing*

![](https://substackcdn.com/image/fetch/$s_!67XG!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F14200e92-bb26-47b1-89d4-c52dc20cccd8_1500x1852.png)

*开盖的 AmpereOne CPU。来源：Brendan Crain，Wikimedia*

![](https://substackcdn.com/image/fetch/$s_!DWkM!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F9c471082-f099-457c-bc01-5aa2deb081cb_2923x1573.png)

*AmpereOne 网格架构。来源：Ampere Computing，Hot Chips 2024*

Ampere Computing 是商业化 ARM 芯片最初的领军者，作为 OEM（原始设备制造商）服务器构建的第三方芯片供应商，直接与 AMD 和英特尔展开竞争。凭借与 Oracle 的紧密合作，Ampere 大张旗鼓地推出了 80 核的 Altra 与 128 核的 Altra Max 系列 CPU，承诺用高性价比的 ARM CPU 打破 x86 CPU 的双头垄断。Ampere Altra 采用 Neoverse N1 内核与自研网格互连架构，将内核划分为多个 4 核集群。与内核配套的是单颗台积电 7nm 裸片上的 8 通道 DDR4 和 128 条 PCIe 4 通道。

得益于转向 5nm 工艺以及新颖的芯粒设计，下一代 AmpereOne CPU 将核心数量提升至 192 个。该设计将 I/O 解耦为独立的 DDR5 和 PCIe 裸片，采用无需使用中介层的 MCM 配置。Ampere 还转向了定制 ARM 内核，该内核追求核心密度而非极致性能，并搭配超大容量的 2MB 二级缓存，以尽量减少来自“吵闹的邻居 (资源抢占者)”的性能损失——即运行在相邻内核上的其他虚拟机抢占共享网格互连流量的问题。类似的 4 核集群被部署在 9x8 网格网络上。总体而言，其整数性能比 Altra Max 翻了一番。

这种芯粒设计允许同一计算裸片在其他变体中重复使用，例如 12 通道的 AmpereOne-M 就增加了 2 颗内存控制器裸片。未来的 AmpereOne-MX 复用了相同的 I/O 芯粒，但替换为拥有 256 个核心的 3nm 计算裸片。其 2024 年路线图还详细介绍了一款未来的 AmpereOne Aurora 芯片，该芯片拥有 512 个核心，并具备 AI 训练和推理能力。

然而，随着 Ampere Computing 在 2025 年被软银以 65 亿美元收购，这份路线图已彻底作废。尽管孙正义确实希望利用 Ampere 的 CPU 设计人才来加强其“星际之门项目”的 CPU 设计，但促成此次收购的另一大动因是 Oracle 急于剥离这项表现不佳的业务。由于时机把握与执行层面的双重问题，Ampere 的 CPU 出货量始终未能实现真正意义上的规模化。

Altra 代产品是他们首次大规模进军市场之作，但由于当时大多数软件并非 ARM 原生，其问世时间对大规模普及而言为时尚早。超大规模云厂商可以迅速调整内部工作负载以适配自研 ARM 芯片，但通用与企业级 CPU 市场的转型步伐要缓慢得多。紧接着，AmpereOne 代产品遭遇多次延期，直到 2024 年下半年 Oracle Cloud A2 实例和 CPU 才姗姗来迟。到那时，超大规模云厂商的 ARM CPU 项目已全面铺开，且 AMD 也能追平 Ampere 的 192 核配置，单核性能更是高出 3 到 4 倍。尽管 Oracle 极力推销单核授权费减半的 Ampere 实例，但这些 CPU 依然无人问津，订单也随之枯竭。 Oracle 从未用完他们购买 Ampere CPU 的全额预付款，其 Ampere CPU 采购额从 2023 财年的 4800 万美元一路陡降至 2024 年的 300 万美元，2025 年也仅为 370 万美元。

归入软银麾下后，Ampere 目前正同时研发 AI 芯片与 CPU。

## ARM Phoenix

![](https://substackcdn.com/image/fetch/$s_!6xHo!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F44e44d5f-8aeb-4974-9868-cd834fe74993_2560x1440.png)

*ARM 的 CSS 产品在定制化与开发成本之间取得平衡。来源：ARM*

ARM 的核心 IP 授权业务在数据中心市场大获成功。几乎所有超大规模云厂商都在其定制 CPU 中采用了 ARM 的 Neoverse 计算子系统 (CSS) 设计。迄今为止，已有超过 10 亿个 Neoverse 核心部署于数据中心 CPU 和 DPU 中，12 家公司共签署了 21 份 CSS 授权协议。随着核心数量不断增加以及超大规模云厂商大举部署 ARM CPU，其数据中心版税营收实现同比翻番有余。ARM 预计，未来几年 CSS 将贡献超过 50% 的版税营收。阅读我们此前的文章，深入了解 ARM 的商业模式以及 CSS 如何攫取更多商业价值。

- [《Arm 的代价：Arm 如何榨取自身的真正价值》](https://newsletter.semianalysis.com/p/arm-and-a-leg-arms-quest-to-extract) - Dylan Patel、Myron Xie 及另外 2 人 · 2023 年 9 月 14 日

然而，ARM 将在 2026 年更进一步，直接提供完整的数据中心 CPU 设计，Meta 将成为其首发客户。这款代号为 Phoenix 的 CPU 将彻底改变 ARM 的商业模式。ARM 将借此转型为芯片供应商，包揽从核心到封装的整颗芯片设计。这意味着 ARM 势必与那些获得 Neoverse CSS 架构授权的客户展开直接竞争。作为软银控股的企业，ARM 还在为 OpenAI 设计定制 CPU，这也是星际之门 (Stargate) OpenAI 与软银合资项目的一部分。此外，Cloudflare 也有望成为 Phoenix 的客户。我们在 [Core Research](https://semianalysis.com/core-research/) 中详细剖析了其销货成本 (COGS)、利润率与营收。

Phoenix 采用了标准的 Neoverse CSS 设计，其物理布图与微软 Cobalt 200 类似。128 个 Neoverse V3 核心通过 ARM 的核心网格网络 (CMN) 互联。这些核心分布在两颗半掩膜尺寸的裸片上，采用台积电 3nm 制程节点制造。在内存与 I/O 方面，Phoenix 配备 12 通道 DDR5（速率为 8400 MT/s）以及 96 条 PCIe 6.0 通道。其能效表现极具竞争力，CPU 的可配置热设计功耗 (TDP) 介于 250W 至 350W 之间。

借此，Meta 如今也拥有了自家的 ARM CPU，足以与微软、谷歌和 AWS (亚马逊云科技) 旗鼓相当。作为 AI 头节点，Phoenix 能够通过加速器支持套件 (Accelerator Enablement Kit)，经由 PCIe 6.0 为外接的 XPU 提供一致性共享内存。下文我们将为订阅用户详细解析下一代 ARM Venom CPU 设计，其中包括一项重大的内存架构变更。

## 华为鲲鹏

中国国产 CPU 的研发进程正在快速推进，龙芯和阿里巴巴的倚天系列均提供了本土设计的选项。然而，该市场的头号玩家当属华为，他们凭借 Kunpeng 处理器系列重新调整了数据中心 CPU 路线图。华为海思团队拥有业内顶尖的芯片设计工程师，其定制的 TaiShan CPU 核心与数据互连架构（data fabric）值得密切关注。

华为最初几代数据中心 CPU 均采用标准的移动端 ARM Cortex 核心。2015 年推出的 Hi1610 搭载了 16 个 A57 核心。2016 年的 Hi1612 将核心数翻倍至 32 个，而 2017 年的 Kunpeng 916 则将核心架构升级为 Cortex-A72。这三代产品均由台积电 16nm 工艺代工。

![](https://substackcdn.com/image/fetch/$s_!nuLP!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F9050cd76-7b26-4fa8-b999-8b5a81a3a501_1306x2336.png)

*Kunpeng 920 裸片图。来源：万扯淡*

Kunpeng 920 于 2019 年问世，采用了极具野心的多芯粒设计，并搭载 64 个定制核心。两颗基于台积电 7nm 工艺的计算裸片，每颗包含 8 个集群，每个集群拥有 4 个基于 ARM v8.2 指令集架构（ISA）运行的 TaiShan V110 核心。这些集群通过环形总线与同一裸片上的 4 通道 DDR4 内存相连，两颗计算裸片共计提供 8 通道内存。Kunpeng 920 是首款采用台积电 CoWoS-S 先进封装的 CPU。它利用一块大型硅中介层，通过定制的裸片间接口，将两颗计算裸片与一颗 I/O 裸片相连；该 I/O 裸片配备了 40 条 PCIe Gen 4 通道，并集成了双 100G 以太网控制器。 尽管 Kunpeng 920 集成了多项前沿技术，但美国对华为的制裁切断了其台积电代工渠道，直接打乱了华为的 CPU 路线图，导致下一代 Kunpeng 930 未能在 2021 年如期发布。

![](https://substackcdn.com/image/fetch/$s_!J5oo!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fc84b6c66-8fe3-4f33-812d-aa3ddfd7c144_1300x1833.png)

*Kunpeng 920B 裸片图。来源：Kurnal*

取而代之的是，升级版的 Kunpeng 920B 于 2024 年悄然发布，并带来了多项升级。TaiShan V120 核心现已支持 SMT，两颗计算裸片上各有 10 个 4 核集群，共计 80 核 160 线程。核心互连和物理布图与 Kunpeng 920 保持相似，计算裸片上配备 8 通道 DDR5。I/O 裸片现被一分为二，计算裸片置于中间。我们认为，两代 CPU 之间长达 5 年的断档，归咎于美国制裁以及被迫针对中芯国际 N+2 工艺重新设计芯片。

![](https://substackcdn.com/image/fetch/$s_!ZzDl!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Ffa36c618-5da3-4cc3-9235-a3b4f5322892_3035x1034.png)

*华为 Kunpeng CPU 路线图。来源：华为*

![](https://substackcdn.com/image/fetch/$s_!r1Rj!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F651c2acf-e470-4797-b124-9cdd060ca65d_3065x778.png)

*华为 TaiShan 950 SuperPoD。来源：华为*

面向 2026 年，华为将推出 Kunpeng 950 再次更新其 CPU 产品线，并将其配置在 TaiShan 950 SuperPoD 机架中以满足通用计算需求。Kunpeng 950 号称在使用其专有的 GaussDB 多写分布式数据库架构时，其 OLTP 数据库性能较 Kunpeng 920B 实现了 2.9 倍的跃升。为实现这一目标，其核心数量翻了一番以上，飙升至 192 核，并采用了保留 SMT 支持的全新 LinxiCore。此外，华为还将推出一款规模较小的 96 核版本。每个 TaiShan 950 SuperPoD 机架内容纳 16 台双路服务器，配备高达 48TB 的 DDR5 内存，这表明其采用了 12 通道内存设计。这些机架还集成了存储与网络功能，将被甲骨文 (Oracle) 的 Exadata 数据库服务器采用，并广泛应用于中国金融领域。 该设计很可能采用中芯国际的 N+3 工艺制造，该工艺近期已在 Kirin 9030 智能手机芯片上首发亮相。

华为的路线图进一步延伸至 2028 年的 Kunpeng 960 系列。这一代顺应了将设计拆分为两大分支的行业趋势。其中，面向 AI 头节点和数据库打造的 96 核 192 线程高性能版本，号称单核性能提升 50% 以上；而面向虚拟化和云计算的高密度版本，则会将核心数量推高至 256 核，甚至可能更多。我们预计，届时华为将在中国超大规模云厂商的 CPU 部署中占据可观的市场份额。

下文将展示我们直至 2028 年的 CPU 路线图，并详细剖析 2026 年之后数据中心 CPU 的核心特性与架构演进。具体内容涵盖：AMD 的 Verano 与 Florence 处理器；英特尔的 Coral Rapids 及其已被彻底砍掉的 CPU 产品线；ARM 的 Venom 规格参数；以及高通凭借 SD2 重返数据中心 CPU 市场的动向。此外，我们还将英伟达的 Bluefield-4 纳入其中，以此作为未来 CPU 部署演进方向的风向标。随后，我们将探讨 DRAM 短缺对各个数据中心 CPU 细分市场的影响，展望未来的 CPU 发展趋势，并重点解析那些将在未来十年重塑 CPU 格局的关键设计要素。

## CPU 路线图

![](https://substackcdn.com/image/fetch/$s_!2Qiq!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Ff3ca1ac7-cb64-4cc9-a9aa-ddf489ab4537_7534x1911.png)

*SemiAnalysis 2017-2028 年 CPU 路线图。来源：SemiAnalysis 预测*

我们的数据中心 CPU 路线图横跨 2017 至 2028 年这 12 年，涵盖了 9 家企业，包括 AMD、英特尔、英伟达、Ampere、AWS、谷歌、微软、ARM 和华为。面向客户的完整版路线图还收录了高通的数据中心 CPU。随着收购案的推进，Ampere Computing (安培计算) 的产品线将面临调整，以期与 OpenAI 产生交集。

## AMD EPYC Verano 与 Florence

![](https://substackcdn.com/image/fetch/$s_!vm-p!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fa49ab8ae-a31f-4ca0-b522-8bfa4c30a0fc_2671x1489.png)

*AMD 机架级 AI 基础设施路线图。来源：AMD*

AMD 下一代 MI500 AI 机架将于 2027 年发布，并搭载全新的霄龙 (EPYC) Verano CPU，距 Venice 问世仅相隔一年。据我们了解，Zen 7 CPU 内核架构赶不上 Verano 的发布节奏，因为内核团队的研发周期约为两年。因此我们认为，Verano 将是 Venice 的变体，继续沿用 Zen 6 内核，且极有可能保持相同的 256 核配置。Verano 的不同之处应在于采用全新的 3nm I/O 裸片，支持 PCIe Gen7 和 200G 以太网 SerDes (串行器/解串器)。这能大幅提升连接 MI500 GPU 的 Infinity Fabric 速度。这也将支持 UALoE。

代号为 Florence 的真正下一代 Zen 7 霄龙 (EPYC) 预计将于 2028 年问世，采用台积电 A16 工艺并支持背面供电。或者，如果不使用背面供电，AMD 也可以等待台积电 A14 工艺成熟，将其用于 2029 年的产品。我们预计其核心数量将增加至 320 个左右。

![](https://substackcdn.com/image/fetch/$s_!otrQ!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fe4a3cd7a-72fe-4414-bcf7-9a1ac1d827f2_3111x1740.png)

*AMD Zen CPU 内核路线图。来源：AMD*

在内核微架构方面，AMD 首席技术官 Mark Papermaster 证实，Zen7 引入了全新的专用矩阵乘法引擎以处理本地 AI 计算。该引擎在 x86 咨询小组中被称为 ACE。这与英特尔在 2023 年随 Sapphire Rapids 引入的 AMX (高级矩阵扩展) 引擎极为相似。Zen7 还采用了 AVX10。作为 AVX-512 的演进版本，它在更小位宽下提供了更多功能与更高的指令灵活性。FRED (灵活返回与事件交付) 中的全新中断模型以及 ChkTag 内存标记技术也将在 Zen7 上首度亮相。所有这些特性都将率先在英特尔 Diamond Rapids 上推出。此外，Diamond Rapids 还支持 APX (高级性能扩展)。这种指令集扩展增加了对更多寄存器的访问权限，并提升了通用计算性能。 Zen7 似乎并未包含该特性。

## 英特尔至强 Coral Rapids

随着陈立武的加入以及 8 通道 Diamond Rapids-SP 被取消，英特尔希望加速 Coral Rapids 的研发进程，推动其尽早上市。在此之前，英特尔将发布搭载 NVLink-C2C 芯粒的定制版至强 CPU，为英伟达的机架级 GPU 设计提供 x86 架构的替代方案。除此之外，陈立武精简计划还有另一个牺牲品：英特尔 Clearwater Forest 的后续产品已被彻底砍掉。正如前文所述，E 核 (能效核) 至强产品线本就停滞不前，该系列在推出仅两代后便将宣告终结。这款后续产品最初代号为 Rouge River Forest，后更名为 DMR-HD (Diamond Rapids 高密度版)；其设计复用了 Diamond Rapids 上的 IMH 裸片，并搭配全新的 CBB 裸片，每个 CBB 裸片包含 128 个 E 核，从而在单插槽中实现高达 512 个核心。

![](https://substackcdn.com/image/fetch/$s_!iJ7C!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fcc3f607a-592a-41cd-980f-ef5692a9247a_1920x1003.png)

*英特尔 18A-Ultra 与 14A-P。来源：英特尔*

针对 Coral Rapids，英特尔已意识到此前的失误，并在其 P 核架构中重新引入了 SMT 技术。我们认为，该架构最初专为英特尔的 14A 制程节点 (P1280) 设计，但该节点要到 2027 年才会进入风险试产阶段；这意味着产品最早也要到 2028 年底才能发布，甚至可能推迟至 2029 年初。为了加速研发并让 Coral Rapids 尽早上市，英特尔可能不得不将该设计移植到经过改进的 18A-Ultra 制程节点上。

## ARM Venom

ARM 继 Phoenix 之后的下一代产品代号为 Venom，将于 2027 年问世。该 CPU 包含 228 个未来的 Neoverse V4 核心，这些核心很可能源自 ARM 的 C1 Ultra 移动端核心。这些核心将采用台积电 N2 制程制造，所有核心均集成在一块光罩极限尺寸的计算裸片上，四周环绕着多个内存和 PCIe 7.0 I/O 裸片，其物理布图与英伟达 Vera CPU 类似。索喜科技参与了部分设计工作，该公司曾于 2023 年与 ARM 和台积电合作，设计了一款基于 N2 制程的 32 核 CPU 芯粒。

值得注意的是，最大的变化在于其放弃了传统的 DDR5 DIMM，转而采用 LPDDR6 内存，这再次效仿了英伟达的设计路线。此举有助于提升封装密度，并实现与 AI XPU 更紧密的集成。此外，该架构采用基于 PCIe 7.0 的新一代加速器支持套件（AEK），用于连接 XPU 并实现一致性内存扩展。

## 高通 SD2

![](https://substackcdn.com/image/fetch/$s_!InJM!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fffd673fa-3259-4f81-837b-7359bb7a1fb9_1920x755.png)

*高通数据中心 CPU 发布会。来源：高通，ServeTheHome*

高通将于 2027 年重返数据中心 CPU 市场，试图通过数据中心 CPU 和 AI 处理器，摆脱对移动和基带业务的单一依赖，实现多元化发展。早在 2017 年，高通就曾携 Centriq 2400 CPU 初次涉足该领域，这款芯片搭载了 48 个 Falkor ARMv8 核心，并配备 6 通道 DDR4 内存。然而，这款产品生不逢时，当时工作负载尚未从 x86 移植到 ARM 的 ISA (指令集架构) 上，导致其普及受限，最终该项目无疾而终。

2021 年，高通收购了 NUVIA，将 Gerard Williams III 及苹果的 CPU 设计人才招致麾下，并立即开始研发代号为 SD1 的数据中心 CPU。虽然这款芯片从未投入量产，但我们掌握了它的规格。SD1 基于台积电 N5 工艺，搭载了 80 个 Oryon 核心（该核心现已应用于骁龙 X Elite 和 8 Elite 移动处理器），配备 16 通道 DDR5-5600 内存和 70 条 PCIe Gen 5 通道，采用尺寸高达 98 x 95 毫米的庞大 LGA9470 插槽，并支持双路配置。这款 CPU 最终未能发布，可能与 2022 年 ARM 对高通提起的诉讼有关。

2025 年，高通正式宣布重返数据中心 CPU 市场，声明将为沙特阿拉伯的 HUMAIN AI 项目研发并供应自家 CPU。我们将这一代 CPU 暂定代号为“SD2”。高通出现在了英伟达 NVLink Fusion 的发布名单中，并确认 SD2 将支持 NVLink Fusion，以此作为连接 AI GPU 的高速一致性互连总线。高通在 2025 年收购 Alphawave Semi，也将进一步巩固其在高速 SerDes (串行器/解串器) 和芯粒互连方面的技术实力。在最新的 2025 财年第四季度财报电话会议上，高通确认 SD2 将于 2027 年出货并贡献营收，并称数据中心 CPU 市场是一个价值数十亿美元的巨大机遇。

然而，Gerard Williams III 及其他 NUVIA 初创团队成员于 2026 年 1 月决定离开高通，这为高通 CPU 项目的未来蒙上了一层阴影。我们认为，由于核心设计团队已完成全面整合，在 2028 年之前，此举带来的负面影响将十分有限。此外，高通在与 ARM 的诉讼中大获全胜，意味着其将继续保有 ARM 授权，并能继续研发定制 CPU 核心架构。

## Bluefield-4 上下文内存存储平台

![](https://substackcdn.com/image/fetch/$s_!cMe7!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F3676208d-dd25-42ba-8f03-cc1359a1076c_2029x1057.png)

*英伟达 Bluefield-4 上下文内存存储平台。来源：英伟达 CES 2026*

![](https://substackcdn.com/image/fetch/$s_!dd72!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fd1e305db-6eb6-4ac4-af46-c11eb9b67b29_2506x1673.jpeg)

*Bluefield-4 封装渲染图。来源：英伟达，SemiAnalysis 预估*

CPU 在网络栈中的比重日益增加。此前的智能网卡（SmartNIC）和数据处理器（DPU）仅在以太网控制器中加入少量 ARM 内核，用于执行网络内卸载与加速。如今，英伟达凭借最新款 DPU 将这一设计推向了新高度。在 Bluefield-4 DPU 中，英伟达直接将一整颗 Grace CPU 与 ConnectX-9 芯片进行共封装。每个 Rubin 计算托盘均将该 DPU 用作前端网卡；此外，英伟达还在其上下文内存存储平台中配置了 4 颗 BF4，专门用于将 KV-Cache 卸载至高速 NAND 闪存。

64 个启用的 Grace 内核搭配 128GB LPDDR5 内存，释放出庞大算力。这表明 CPU 正深度介入 AI 数据管理。当前的 AI 网络拓扑包含东西向（用于 GPU 集群的横向扩展）和南北向（用于连接存储、外部资源与互联网）。模型 KV-Cache 的卸载与存储，则相当于在现有架构中新增了第三张网络。随着 Bluefield-4 的问世，具备 CPU 算力的网卡与具备网络加速能力的 CPU（如至强 6 SoC Granite Rapids-D）之间的界限愈发模糊。

## DRAM 对 CPU 市场的制约

到了 2026 年此时，供应链正面临服务器 CPU 与大宗 DDR5 内存双双严重短缺的局面。服务器 CPU 供货配额将优先分配给能证明自身拥有稳定内存供应的客户。这一趋势将加速市场向 OpenAI、CoreWeave 及云服务巨头等大型玩家集中，因为他们拥有庞大的 DRAM 采购量与极强的议价能力。全面袭来的 CPU 涨价潮叠加高昂的内存价格，将对数据中心 CPU 市场的各大细分领域产生截然不同的影响。

对于单插槽配备 512GB 至 1TB 内存的 AI 头节点而言，内存的成本与供货难度，远超 CPU 及服务器机箱其余组件。未来，若 CPU 供应好转而内存依然紧缺（产能继续被高毛利的 AI 加速器大肆吞噬），该细分市场受到的冲击将最为惨烈。CPU 厂商极度担忧内存重复下单会加剧表象上的内存短缺。为此，他们正严密监控并预测库存水平，力求在持续的内存压力下妥善管控近期 CPU 需求。

在云计算与虚拟化市场，CPU 的吞吐量与能效是首要考量，且大内存实例的租赁定价更高。由于该细分市场的内存支出占比相对较低，其受内存价格波动的影响也较小。至于存储与边缘服务器，通常采用核心数较少的低端 CPU，CPU 的平均售价 (ASP) 仅占系统总成本的极小部分。短期内，CPU 供货可能会从低端市场抽离。在产能受限的环境下，CPU 厂商势必优先保供高端、高毛利的 SKU，以实现营收最大化。

## 未来 CPU 发展趋势

随着计算需求随时间推移不断变化，看似不起眼的 CPU 在未来十年仍将持续演进。当前强化学习 (RL) 训练与智能体工具应用的时代，正以前所未有的速度推高 CPU 需求，CPU 设计也开始分化，以适应三大应用场景。

- 面向云计算与通用计算，具备高吞吐量的高核心密度、高能效 CPU（如 Graviton5）

- 单核性能强劲、具备高内存带宽，且与 AI 加速器紧密集成以实现一致性内存扩展的 CPU（如 Vera）

- 具备数据处理与网络能力，用于应对 AI 集群海量数据需求及模型上下文卸载的 CPU 和 DPU（如 Bluefield-4）

展望未来，我们很可能会看到更高程度的专业化分工。无论哪种场景，CPU 插槽规模与核心数量都在不断增加，内存与 I/O 的速度及位宽也在持续提升，这给 CPU 互连结构与数据网络带来了更大压力。业界将更加依赖片上网络 (NoC) 设计师。他们需要跨越数千平方毫米的硅片面积与多个芯粒边界，打造具备极高对分带宽的设计；同时还要保持与外部设备的完全一致性，并满足远低于 GPU 网络的延迟要求。

数据中心将继续采用 LPDDR。针对大型 16 通道内存设计，PCB 的层数与复杂度已被推至极限。LPDDR6 SOCAMM 模块能够实现高得多的带宽密度，同时保持比封装内内存更高的容量。

最终胜出的设计必将拥有最优秀的数据网络。一些设计将采用数 GB 的缓存来隐藏延迟并提升内存受限型性能；而另一些设计则会大举采用 CXL 内存扩展器。我们甚至可能看到 HBM 重返 CPU，这一设计上一次出现还是在 Sapphire Rapids 至强 Max 与 AMD 的 MI300C 中。

放眼更长远的未来，我们今天所熟知的数据中心 CPU 还会存在吗？下一个计算需求时代可能会转向由 AMD MI300A 开创的加速处理器 (APU) 设计。这类设计集成了 CPU 核心，从而消除了对 CPU 头节点的需求。对于 RL 训练专用加速器而言，APU 也可能成为一种选择。它们可以利用统一内存，在本地运行 RL 环境，从而缩短 RL 任务往返通用 CPU 集群的时间。共封装光学器件、结合 JBOM（海量 HBM 阵列）概念的内存解耦与池化技术，以及 Credo 的 Omniconnect 内存变速器，可能会降低对内存扩展 CPU 的需求，并降低 CPU 配比率（例如每个加速器机架仅配 1 个 CPU）。我们还预见在未来，CPU 最终会作为核心组件，被集成到超过 400T 的巨型交换机中。 无论如何，CPU 都将继续作为计算机的基础核心存在。

请联系 sales@semianalysis.com 获取以下数据。

- 物料清单 (BOM) 成本分析

- 提供一份表格，详细拆解 2026 年 x86 数据中心 CPU 的生产成本。针对每个处理器系列，将提供 SKU 堆栈中不同产品层级的成本数据。

- 具体型号包括：采用经典与高密度 CCD 配置的 AMD Venice SP7 (插槽) 和 SP8 变体；英特尔 Diamond Rapids-AP 超高核心数 (UCC) 16 通道版本；以及 Diamond Rapids-SP 高核心数 (HCC) 与低核心数 (LCC) 8 通道版本。

- 包含英伟达 Grace 与 Vera 架构 ARM CPU，以及其他已讨论的 ARM CPU 的物料清单成本，作为与上述 x86 CPU 的对比基准。

- 成本拆解涵盖：有源硅（包括 CPU、SoC 与 I/O）、先进封装（2.5D 与混合键合）、传统封装成本（包含良率影响），以及产品测试成本。

- 有源硅成本综合考量了所有裸片尺寸、工艺技术与金属层数、单片晶圆良品裸片数、晶圆成本以及单封装裸片数。例如，顶配 256 核 AMD Venice SP7 (插槽) 版本的有源硅成本，将基于 8 颗计算裸片的台积电 N2 晶圆成本，加上 2 颗 I/O 裸片的台积电 N6 晶圆成本计算得出。

- 先进封装成本考量了封装类型（例如 AMD Venice 采用的 CoWoS-L，英特尔 Diamond Rapids 采用的 EMIB (嵌入式多芯片互连桥接) 和 Foveros Direct 3D）、中介层面积与混合键合尺寸、裸片间互连数量以及已知良好裸片 (KGD) 测试。先进封装工艺造成的良率损失，会导致完好裸片因封装组件失效而被浪费，这部分损失将计入额外的有源硅成本。

- 传统封装成本包含基板成本（综合考量材料、层数与面积），以及集成无源器件、热界面材料 (TIM) 和顶盖成本。

- 测试成本包含封装级的自动测试设备 (ATE) / 系统级测试 (SLT) 以及老化测试成本。芯片分级 / 筛选过程中的损耗，会推高单封装的平均物料清单成本。

- 架构设计与性能分析

- 列出 AMD Venice 与英特尔 Diamond Rapids 的完整规格，包括但不限于：核心数、TDP (热设计功耗)、缓存大小、内存带宽、PCIe 与 CXL 的 IO 通道数，以及多插槽互连规格（AMD 的扩展全局内存互连 (XGMI) 与英特尔的超级通道互联 (UPI)）。

- 提供 CPU 物理布图与拓扑结构图，涵盖核心与末级缓存 (LLC) 布局、网格互连尺寸、裸片间 PHY 以及 IO PHY。

- 分析物理布图与拓扑结构及其在历代 CPU 中的演进，并点评将网格架构扩展至大规模时面临的挑战。

- 对比 AMD、英特尔与 ARM 的布局设计，并探讨其对 NUMA 域以及具有多个 LLC 域的核心集群的影响。

- 提供先进封装的集成示意图，包括 chiplet 布局、中介层尺寸以及凸点间距。

- 探讨先进封装技术的选择 (CoWoS-L (AMD Venice)、CoWoS-R (英伟达 Vera) 与 EMIB (嵌入式多芯片互连桥接)/Foveros Direct 3D (英特尔 Clearwater Forest))，及其对成本、功耗与性能的影响

- 详述高核心数 CPU 设计面临的可扩展性挑战，及其对核心间延迟、内存访问一致性与延迟的影响

- 提供 AMD Venice 与英特尔 Diamond Rapids 的内部性能预估数据 (如 SPECint2017)，并分析性能随核心数与芯片成本变化的可扩展性

...
export const ChatBubble: FC<ChatBubbleProps> = ({
  ...
  // 中文润色并添加
  const handleComposeAndAdd = async () => {
    ...
    try {
      ...
      addMessage({ role: MessageRole.ME, content_en: result.professional, content_cn: customInput.trim() });
      await slackApi.addMessage(conversationId, { role: MessageRole.ME, content_en: result.professional, content_cn: customInput.trim() });
      setCustomInput('');
      setIsExpanded(false);
      showSuccess('回复已添加');
    } catch {
        ...
        <div className="mt-2 rounded-lg border border-border-subtle bg-bg-card p-3 shadow-sm">
          ...
          {!showSuggestions && (
            <div className="border-t border-border-subtle pt-3 mt-3">
              <div className="text-sm text-text-muted mb-2">✏️ 输入中文，AI润色成英文：</div>
              <Textarea
                value={customInput}
                onChange={(e) => setCustomInput(e.target.value)}
                placeholder="输入你想说的中文..."
              ...
              <Button
                size="sm"
                variant="primary"
                onClick={handleComposeAndAdd}
                disabled={!customInput.trim() || isComposing}
